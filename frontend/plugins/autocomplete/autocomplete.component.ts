...
import { catchError, debounceTime, exhaustMap, filter, map, startWith, switchMap, tap } from 'rxjs/operators';
...
} from '@appRoot/visa/externalVisaWorkflow/createExternalUser/create-external-user.component';
...
export class PluginAutocompleteComponent implements OnInit {
    ...
    type: { user: string, entity: string, contact: string } = {
        user: 'fa-user',
        entity: 'fa-sitemap',
        contact: 'fa-address-book',
    };

    typeLabel: { user: string, entity: string, contact: string } = {
        user: this.translate.instant('lang.user'),
        entity: this.translate.instant('lang.entityLabel'),
        contact: this.translate.instant('lang.contact')
    };
    ...
    constructor(
        ...
        private externalSignatoryBookService: ExternalSignatoryBookManagerService
    ) {
    }

    ngOnInit() {
        this.appearance = this.appearance === undefined ? 'legacy' : 'outline';
        this.singleMode = this.singleMode !== undefined;
        ...
        if (this.manageDatas !== undefined) {
            ...
            this.dialogRef = this.dialog.open(ConfirmComponent, {
                ...
                data: {
                    ...
                    msg: 'Voulez-vous créer cet élément <b>' + newElem[this.targetSearchKey] + '</b>&nbsp;?'
                }
            });
            ...
            tap(async (data: any) => {
                if (data) {
                    const user = {
                        ...
                        picture: await this.externalSignatoryBookService.getUserAvatar(data.otp.type),
                        hasPrivilege: true,
                        isValid: true,
                        externalId: {
                            [this.externalSignatoryBookService.signatoryBookEnabled]: null
                        },
