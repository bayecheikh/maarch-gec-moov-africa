...
export class PluginSelectSearchComponent implements OnInit, OnDestroy, AfterViewInit, ControlValueAccessor, DoCheck {
    ...
    private _filter(value: string): string[] {
        if (value === '__SELECTED') {
            ...
            const filterValue: string = this.latinisePipe.transform(value.toLowerCase());

            const group: any[] = this.datasClone.filter((option: any) => option['isTitle'] && this.latinisePipe.transform(option['label'].toLowerCase()).includes(filterValue)).map((opt: any) => opt.id);

            return this.datasClone.filter((option: any) => (
                option['isTitle'] &&
                    group.indexOf(option['id']) > -1) ||
                (group.indexOf(option['group']) > -1 || this.latinisePipe.transform(option['label'].toLowerCase()).includes(filterValue)));
        } else {
