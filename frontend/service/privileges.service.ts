...
interface Administration {
    ...
    'hasParams': boolean;
    'subAdminPath'?: string; // Sub admin privilege path
}
...
export class PrivilegeService {
    ...
    private administrations: Administration[] = [
        ...
        },
        {
            ...
            'hasParams': false,
            'subAdminPath': 'administration/issuingSites'
        },
        {
            'id': 'admin_search',
            'label': 'lang.search',
        ...
        },
        {
            ...
            'style': 'fas fa-plug',
            'hasParams': false,
            "subAdminPath": 'administration/connections/proconnect'
        },
        {
        ...
        {
            'id': 'admin_folders',
            'label': 'lang.adminFolders',
            'comment': 'lang.adminFoldersDesc',
            'route': '/administration/folders',
            'unit': 'supervision',
            'style': 'fa fa-folder',
            'hasParams': false
        },
        {
            'id': 'admin_goodflag',
            'label': 'lang.adminGoodflag',
            'comment': 'lang.GoodflagDesc',
            'route': '/administration/goodflag',
            'unit': 'supervision',
            'style': 'goodflag',
            'hasParams': false
        }
    ...
    private privileges: Privilege[] = [
        ...
        {
            'id': 'view_confidential_contact_information',
            'label': 'lang.viewConfidentialContactInformation',
            'comment': 'lang.viewConfidentialContactInformationDesc',
            'unit': 'contact'
        },
        {
            'id': 'update_confidential_contact_information',
            'label': 'lang.updateConfidentialContactInformation',
            'comment': 'lang.updateConfidentialContactInformationDesc',
            'unit': 'contact'
        },
        {
        ...
        {
            'id': 'download_proof_file',
            'label': 'lang.downloadProof',
            'comment': 'lang.downloadProofDesc',
            'unit': 'visaWorkflow'
        },
        {
            'id': 'print_folder_doc',
    ...
    getAllPrivileges(getLockedPrivilege: boolean, authMode: string = 'standard', groups: any[]): Promise<any[]> {
        ...
        priv = priv.filter(elem => this.getExcludePrivilege(getLockedPrivilege, authMode, true).indexOf(elem) === -1);

        return this.getxEcludePrivilegePlugin(priv, groups).then(privileges => privileges);
    }
    ...
    getUnitsPrivileges(): Array<string> {
        return this.getActivePrivileges().map(elem => elem.unit).filter((elem, pos, arr) => arr.indexOf(elem) === pos);
    }


    getPrivilegesByUnit(unit: string): Array<Privilege> {
        return this.getActivePrivileges().filter((privilege) => this.getExcludePrivilege(false, 'standard').indexOf(privilege.id) === -1).filter(elem => elem.unit === unit);
    }
    ...
    hasCurrentUserPrivilege(privilegeId: string) {
        ...
        return privileges.filter((privilegeId) => this.getExcludePrivilege(true, 'standard').indexOf(privilegeId) === -1).indexOf(privilegeId) > -1;
    }

    private getExcludePrivilege(getLockedPrivilege: boolean, authMode: string, initAllPrivileges: boolean = false) {
        let excludePriv = [];
        ...
        if (!this.pluginManagerService.isPluginLoaded('maarch-plugins-ragtime') && !initAllPrivileges) {
            excludePriv.push('ragtime_plugin');
        }
        return excludePriv;
    }

    private async getxEcludePrivilegePlugin(priv: any[], groups: any[]): Promise<any[]> {
        return new Promise(resolve => {
            (async () => {
                const idGroups = groups.map(group => group.id);

                const requestsPrivileges = idGroups.map(id =>
                    this.http.get(`../rest/groups/${id}/details`).pipe(
                        map((data: any) => data.group?.privileges ?? []),
                        catchError(() => of([]))
                    )
                );

                if (idGroups.length > 0) {
                    const results: any[] = await lastValueFrom(forkJoin(requestsPrivileges));
                    const privilegesGroups = [...new Set(results.reduce((acc, val) => acc.concat(val), []))];

                    if (!privilegesGroups.includes('ragtime_plugin')) {
                        priv = priv.filter(item => item !== 'ragtime_plugin');
                    }
                } else {
                    // No group No plugin
                    priv = priv.filter(item => item !== 'ragtime_plugin');
    ...
    private getActivePrivileges(): Privilege[] {
        return this.privileges.filter((privilege) => this.getExcludePrivilege(false, 'standard').indexOf(privilege.id) === -1);
    }
}
