...
export class NotificationService {
    ...
    showMessage(message: string, url: string = '', type: string, icon: string, duration: number): void {
        const snackBar = this.snackBar.openFromComponent(CustomSnackbarComponent, {
            ...
            data: {
                url: url, message: message, icon: icon, close: () => {
                    snackBar.dismiss();
                }
            }
        });
    ...
    handleBlobErrors(err: any) {
        ...
        // Check if the error is a valid Blob
        if (blob instanceof Blob) {
            ...
            reader.onload = (res) => {
                try {
                    ...
                    this.error(`Unknown error`, err.url);
                }
            };
            reader.readAsText(blob);
        } else {
            // Fallback for non-Blob errors (e.g., plain strings or undefined)
            const message = typeof blob === 'string' ? blob : 'Unknown error';
            this.error(`${err.status} : ${message}`, err.url);
        }
    }
