...
export class AppGuard  {
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
        ...
        if (this.appService.coreLoaded) {
            return of(this.handleNavigation(route, state));
        }

        return this.appService.catchEvent().pipe(
            map(() => {
                return this.handleNavigation(route, state);
            }),
            catchError(() => {
    ...
    handleNavigation(route: ActivatedRouteSnapshot, routerState: RouterStateSnapshot): boolean {
...
export class AfterProcessGuard  {
    ...
    async canDeactivate(
        ...
        if (
            ...
            const value: string = await this.getConfirmation();

            if (value === undefined) {
                const secondValue: string = await this.getConfirmation();
                if (secondValue) {
    ...
    getConfirmation(): Promise<string> {
        return new Promise((resolve) => {
            const dialogRef: MatLegacyDialogRef<ConfirmComponent> = this.dialog.open(ConfirmComponent, {
