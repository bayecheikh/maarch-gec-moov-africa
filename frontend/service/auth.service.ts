...
export class AuthService {
    ...
    externalSignatoryBook: { id: string, from?: string, integratedWorkflow?: boolean, optionOtp?: boolean } = { id: '', from: '', integratedWorkflow: false, optionOtp: false };
    ...
    clearTokens() {
        ...
        this.localStorage.remove('Proconnect-idToken');
    ...
    async logout(cleanUrl: boolean = true, forcePageLogin: boolean = false, history: boolean = false) {
        this.clearFilters();
        this.headerService.tiles = undefined;
        if (['cas', 'keycloak', 'sso'].indexOf(this.authMode) > -1 && !forcePageLogin) {
    ...
    SsoLogout(cleanUrl: boolean = true) {
        this.http
            ...
            .pipe(
                tap(async (data: any) => {
                    this.redirectAfterLogout(cleanUrl);
                    if (!this.functionsService.empty(data.logoutUrl)) {
        ...
        return this.http.get('../rest/currentUser/profile').pipe(
            switchMap((data: any) => {
                const baseUser = {
                    ...
                    nbResourcesFollowed: data.nbFollowedResources,
                };
                this.headerService.nbResourcesFollowed = data.nbFollowedResources;

                return data.privileges[0] === 'ALL_PRIVILEGES'
                    ? from(this.privilegeService.getAllPrivileges(!data.lockAdvancedPrivileges, this.authMode, data.groups)).pipe(
                        map(privileges => ({ ...baseUser, privileges }))
                    )
                    : of({ ...baseUser, privileges: data.privileges });
            }),
            tap(user => {
