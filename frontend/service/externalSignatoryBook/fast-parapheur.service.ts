...
export class FastParapheurService {
    ...
    canAddSteps: boolean = false;

    userWorkflow: UserWorkflow = new UserWorkflow();
    signatureModes: string[] = [];
    workflowTypes: { id: string, label: string }[] = [];
    otpConnectors: { id: number, label: string, type: string }[] = [];
    ...
    linkAccountToSignatoryBook(externalId: any, serialId: number): Promise<any> {
        return new Promise((resolve) => {
            this.http.put(`../rest/users/${serialId}/linkToFastParapheur`, {
                fastParapheurUserEmail: externalId.email,
                fastParapheurUserName: externalId.idToDisplay
            }).pipe(
                tap(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    downloadProof(fastParapheurWorkflowId: string): Promise<boolean> {
        return new Promise((resolve) => {
            resolve(true);
        })
    }
