...
export class ExternalSignatoryBookManagerService {
    ...
    allowedSignatoryBook: string[] = ['maarchParapheur', 'fastParapheur', 'goodflag'];
    serviceInjected: MaarchParapheurService | FastParapheurService | GoodflagService;
    signatoryBookEnabled: string = '';
    currentWorkflow: UserWorkflow[] = null;
    ...
    constructor(
        ...
        const externalBook = this.authService.externalSignatoryBook;
        const isAllowed: boolean = this.allowedSignatoryBook.includes(externalBook?.id ?? '');

        if (isAllowed && externalBook.integratedWorkflow) {
            const { id, integratedWorkflow } = externalBook;

            this.signatoryBookEnabled = id;

            const serviceMap = {
                maarchParapheur: MaarchParapheurService,
                fastParapheur: integratedWorkflow ? FastParapheurService : null,
                goodflag: GoodflagService,
            };

            const serviceClass = serviceMap[id];

            if (serviceClass) {
                this.serviceInjected = this.injector.get(serviceClass);
            }
        ...
        resourcesToSign.forEach((resource: any) => {
            workflow.forEach((element: any, index: number) => {
                const step: ResourceStep = {
                    ...
                    'step': element.step,
                    'isSignatureRequired': element.isSignatureRequired
                };
                formatedData['steps'].push(step);
            });
        });

        const element: string = this.signatoryBookEnabled === 'fastParapheur' ? 'workflowType' : 'templateId';
        formatedData[element] = workflowType;
    ...
    canAddSteps(): boolean {
        return this.serviceInjected?.canAddSteps;
    }

    isMappingWithTechnicalStatus(): boolean {
        return this.serviceInjected?.isMappingWithTechnicalStatus;
    }

    async downloadProof(id: string): Promise<void> {
        await this.serviceInjected?.downloadProof(id);
    }
}
