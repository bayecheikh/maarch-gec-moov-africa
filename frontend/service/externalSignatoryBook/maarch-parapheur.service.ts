...
export class MaarchParapheurService {
    ...
    isMappingWithTechnicalStatus: boolean = true;
    ...
    createExternalSignatoryBookAccount(id: number, login: string): Promise<any> {
        return new Promise((resolve) => {
    ...
    isValidParaph(additionalsInfos: any = null, workflow: any[] = [], resourcesToSign = [], userOtps = []) {
        return !(additionalsInfos.attachments.length === 0 || workflow.length === 0 || userOtps.length > 0 || resourcesToSign.length === 0);
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    downloadProof(maarchParapheurWorkflowId: string): Promise<boolean> {
        return new Promise((resolve) => {
            resolve(true);
        })
    }
