...
export class DashboardComponent implements OnInit {
    ...
    ngOnInit(): void {
        if (!this.headerService.tiles) {
            this.headerService.tiles = [];
    ...
    enterTile(tile: any, index: number) {
        ...
        this.headerService.tiles.forEach((element: any, indexTile: number) => {
            element.editMode = indexTile === index;
        });
    }
                ...
                for (let index = 0; index < 6; index++) {
                    if (position === index || position === null) {
                        ...
                        if (!this.functionsService.empty(tmpTile)) {
                            ...
                            if (position !== null) {
                                this.headerService.tiles[index] = objTile;
                            } else {
                                this.headerService.tiles.push(objTile);
                            }
                        } else {
                            if (position !== null) {
                                this.headerService.tiles[index] = {
                                    id: null,
                                    position: index,
                                    editMode: false
                                };
                            } else {
                                this.headerService.tiles.push({
                                    id: null,
                                    position: index,
                                    editMode: false
                                });
    ...
    changeView(tile: any, view: string, extraParams: any = null) {
        ...
        this.http.put(`../rest/tiles/${tile.id}`, tileToSend).pipe(
            tap(() => {
                const indexTile = this.headerService.tiles.filter((tileItem: any) => tileItem.id !== null).map((tileItem: any) => tileItem.position).indexOf(tile.position);
                this.headerService.tiles[tile.position] = tileToSend;
    ...
    changeTileColor(tile: any, color: string) {
        this.headerService.tiles[tile.position].color = color;
        this.http.put(`../rest/tiles/${tile.id}`, this.headerService.tiles[tile.position]).pipe(
            catchError((err: any) => {
                this.notify.handleErrors(err);
                return of(false);
            })
    ...
    onDrop(event: CdkDragDrop<any>) {
        this.headerService.tiles[event.previousContainer.data.position] = event.container.data;
        this.headerService.tiles[event.container.data.position] = event.previousContainer.data;
        transferArrayItem(
    ...
    transferDataSuccess() {
        this.headerService.tiles.forEach((tile: any, index: number) => {
            tile.position = index;
        });
        this.http.put('../rest/tilesPositions', { tiles: this.headerService.tiles.filter((tile: any) => tile.id !== null) }).pipe(
            catchError((err: any) => {
                this.notify.handleErrors(err);
                return of(false);
            })
    ...
    delete(tile: any) {
        ...
        dialogRef.afterClosed().pipe(
            ...
            tap(() => {
                this.headerService.tiles[tile.position] = {
                    id: null,
    ...
    emptyDashboard() {
        return this.headerService.tiles.filter((item: any) => item.id !== null).length === 0;
    }
