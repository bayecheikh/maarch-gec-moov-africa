...
import { BasketGroupListActionInterface } from "@models/actions.model";
...
export class ListAdministrationComponent implements OnInit {
    ...
    availableValidationsActions: BasketGroupListActionInterface[] = [];
    availableValidationsActionsClone: BasketGroupListActionInterface[] = [];

    availableRefusalActions: BasketGroupListActionInterface[] = [];
    availableRefusalActionsClone: BasketGroupListActionInterface[] = [];
    ...
    async ngOnInit(): Promise<void> {
        ...
        if (this.currentBasketGroup.list_event === 'processDocument') {
            ...
            this.selectedProcessTool.canAddAttachments = this.currentBasketGroup.list_event_data === null ? false : this.currentBasketGroup.list_event_data.canAddAttachments;
    ...
    moveAllActions(source: string, target: string, type: string, ): void {
        this[source].forEach((action: BasketGroupListActionInterface) => {
            action['type'] = type;
    ...
    moveAction(action: BasketGroupListActionInterface, arraySource: string, arrayTarget: string, type: string): void {
        action.type = type;
    ...
    setActionsChosen(action: BasketGroupListActionInterface = null): Promise<boolean> {
        return new Promise((resolve) => {
            ...
            this.currentBasketGroup = this.selectedGroup;
