...
import { ActionsAdminListInterface } from "@models/actions.model";
...
export class BasketAdministrationComponent implements OnInit {
    ...
    loadingListAdmin: boolean = false;
                    ...
                    .subscribe((data: any) => {
                        ...
                        this.http.get('../rest/baskets/' + this.id + '/groups')
                            .subscribe((dataGroups: any) => {
                                ...
                                dataGroups.groups.forEach((tmpGroup: any) => {
                                    tmpGroup.groupActions.forEach((tmpAction: ActionsAdminListInterface) => {
                                        tmpAction.used_in_basketlist = tmpAction.used_in_basketlist as any === 'Y';
                                        tmpAction.used_in_action_page = tmpAction.used_in_action_page as any === 'Y';
                                        tmpAction.default_action_list = tmpAction.default_action_list as any === 'Y';
                                    });
                                });
    ...
    setDefaultAction(group: any, action: any) {
        group.groupActions.forEach((tmpAction: ActionsAdminListInterface) => {
            tmpAction.default_action_list = tmpAction.id === action.id;
        });
    ...
    addAction(group: any) {
        this.http.put('../rest/baskets/' + this.id + '/groups/' + group.group_id + '/actions', { 'groupActions': group.groupActions }).pipe(
            tap(() => {
                ...
                if (this.signatureBookService.config.isNewInternalParaph && (group.list_event === 'signatureBookAction' || this.listAdmin.selectedListEvent === 'signatureBookAction')) {
                    setTimeout(() => {
                        this.listAdmin.selectedGroup.groupActions = group.groupActions;
    ...
    toggleIsSearchBasket(basket: any) {
        ...
        this.http.put('../rest/baskets/' + this.id, this.basketClone).pipe(
            tap(() => {
                this.notify.success(this.translate.instant('lang.basketUpdated'));
            }),
            catchError((err: any) => {
    ...
    onTabChange(index: number): void {
        ...
        this.loadingListAdmin = true;
        if (index > 0) {
            this.selectedGroup = this.basketGroups[index - 1];
            setTimeout(() => {
                this.loadingListAdmin = false;
            }, 0);
        }
    }

    onTabActionsChange(index: number) {
        if (index === 1) {
            this.onTabChange(this.selectedGroupIndex);
        }
    }
}
...
export class BasketAdministrationGroupListModalComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.http.get('../rest/actions')
            .subscribe((data: any) => {
                data.actions.forEach((tmpAction: ActionsAdminListInterface) => {
