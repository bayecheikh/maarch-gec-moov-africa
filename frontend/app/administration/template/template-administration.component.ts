...
import { filter, tap, catchError, exhaustMap, finalize } from 'rxjs/operators';
                ...
                this.http.get('../rest/templates/' + params['id'] + '/details')
                    .subscribe((data: any) => {
                        ...
                        this.template = {
                            ...
                            signaturePositions: data.template.signaturePositions,
                            file: {}
                        };
                ...
                this.http.post('../rest/templates', this.formatTemplate()).pipe(
                    tap((data: any) => {
                        if (data.checkEntities) {
                            ...
                            const dialogRef = this.dialog.open(TemplateAdministrationCheckEntitiesModalComponent, this.config);
                            dialogRef.afterClosed().pipe(
                                finalize(() => {
    ...
    openSignaturePosition(): void {
        const dialogRef = this.dialog.open(SignaturePositionComponent, {
            ...
            data: {
                template: {
                    id: this.template.id,
                    base64FileContent: this.templateDocView,
                    title: this.template.label
                } as {
                    id: number;
                    base64FileContent: { changingThisBreaksApplicationSecurity: string };
                    title: string;
                },
                signaturePositions: (this.template.signaturePositions ?? []) as SignaturePositionInterface[]
            }
        });
        dialogRef.afterClosed().pipe(
            filter((res: any) => !this.functionsService.empty(res)),
            tap((res: {signaturePositions: SignaturePositionInterface[]}) => {
                this.template['signaturePositions'] = res.signaturePositions;
            }),
