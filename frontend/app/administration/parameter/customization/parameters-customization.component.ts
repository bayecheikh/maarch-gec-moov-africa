...
export class ParametersCustomizationComponent implements OnInit, OnDestroy {
    ...
    backgroundList: { url: string, filename: string }[] = [];
    ...
    initMce(readonly = false): void {
        const param = {
            ...
            readonly: readonly,
            plugins: [
                'autolink', 'table', 'autoresize', 'link',
            ],
            ...
            toolbar: !readonly ? 'undo redo | fontselect fontsizeselect | bold italic underline strikethrough forecolor | link | table maarch_b64image | \
    ...
    uploadTrigger(fileInput: any, mode: string): void {
        if (fileInput.target.files && fileInput.target.files[0]) {
            ...
            if (res === true) {
                ...
                this.notify.error(res as string);
            }
        }
    }

    canUploadFile(file: any, mode: string): boolean | string {
    ...
    clickLogoButton(uploadLogo: any): void {
        if (!this.stepFormGroup.controls['logo'].disabled) {
            uploadLogo.click();
        }
    }

    saveParameter(parameterId: string): void {
        let param = {};
        ...
        this.http.put('../rest/parameters/' + parameterId, param).pipe(
            tap(() => {
