...
import { ActionAdminInfoInterface, ActionPagesInterface } from '@models/actions.model';
...
export class ActionAdministrationComponent implements OnInit {
    ...
    action: ActionAdminInfoInterface = null;
    statuses: any[] = [];
    actionPages: ActionPagesInterface[] = [];
    categoriesList: any[] = [];
    keywordsList: any[] = [];
    ...
    digitalCertificateByDefault: boolean = true;
    ...
    mailevaStatus: { id: string, label: string, actionStatus: string, disabled: boolean }[] = [
        {
        ...
        this.route.params.subscribe(params => {
            if (typeof params['id'] === 'undefined') {
                ...
                this.http.get('../rest/actions/' + params['id'])
                    .subscribe(async (data: any) => {
                        this.action = data.action;
                        const currentAction: ActionPagesInterface = this.actionPagesService.getActionPageByComponent(this.action.component);
                        ...
                        if (this.action.actionPageId === 'confirm_status') {
                            this.customFieldsFormControl = new UntypedFormControl({
                                value: this.action.parameters.fillRequiredFields,
                                disabled: false
                            });
                                ...
                                this.availableFillCustomFields.forEach((availableElement: any) => {
                                    if (availableElement.id === element.id) {
                                        ...
                                        if (availableElement.type === 'date' && !this.functions.empty(element.value)) {
                                            ...
                                            availableElement.formControl = new UntypedFormControl({
                                                value: element.value,
                                                disabled: false
                                            });
                                        }
                            ...
                            this.customFieldsFormControl = new UntypedFormControl({
                                value: this.action.parameters.requiredFields,
                                disabled: false
                            });
                            ...
                            this.filterAbsentUsers = this.action.parameters.filterAbsentUsers ?? false;
                            this.digitalCertificateByDefault = this.action.parameters.digitalCertificateByDefault ?? true;
                            this.inSignatureBook = this.action.parameters.inSignatureBook ?? false;
                        }
                    });
            }
        });
    ...
    getCustomFields() {
        ...
        this.digitalCertificateByDefault = this.creationMode && this.action.actionPageId === 'visa_workflow';
    ...
    getSelectedFieldsToFill() {
        ...
        if (!this.functions.empty(this.selectedValue) && this.functions.empty(checkField) &&
            ...
            if (this.selectedValue.type === 'date') {
                ...
                this.selectedValue.formControl = new UntypedFormControl({
                    value: this.selectedValue.selectedValues,
                    disabled: false
                });
            }
    ...
    selectedContact(contact: any, fieldItemValue: any) {
        ...
        if (!this.functions.empty(fieldItemValue.selectedValues)) {
            ...
            if (this.functions.empty(contactExist)) {
                fieldItemValue.selectedValues.push({
                    id: contact.id,
                    type: contact.type,
                    label: arrInfo.filter(info => !this.functions.empty(info)).join(' ')
                });
            }
        } else {
    ...
    onSubmit() {
        if (this.action.actionPageId === 'confirm_status') {
            ...
            this.action.parameters = {
                ...
                lockVisaCircuit: this.lockVisaCircuit,
                keepDestForRedirection: this.keepDestForRedirection,
                keepCopyForRedirection: this.keepCopyForRedirection,
                keepOtherRoleForRedirection: this.keepOtherRoleForRedirection,
                digitalCertificateByDefault: this.digitalCertificateByDefault ?? true,
                inSignatureBook: this.action.parameters.inSignatureBook ?? false,
            };
        }
        ...
        if (this.action.actionPageId !== 'send_to_visa') {
            ...
            delete this.action.parameters.inSignatureBook;
        }
        if (this.action.actionPageId !== 'redirect') {
            ...
            delete this.action.parameters.filterAbsentUsers;
        }
    ...
    getSelectedStatus(mailevaStatus: any[], actionStatus: string) {
        if (actionStatus === 'intermediateStatus') {
            ...
            this.finalStatusParams.data.forEach((element: any) => {
                element.disabled = !!(mailevaStatus.indexOf(element.id) > -1 || element.disabled);
            });
    ...
    toggleVisaCircuit(action: ActionAdminInfoInterface) {
        this.lockVisaCircuit = action.parameters.lockVisaCircuit;
    }


    toogleKeepDest(action: ActionAdminInfoInterface) {
        this.keepDestForRedirection = action.parameters.keepDestForRedirection;
    }

    toogleKeepCop(action: ActionAdminInfoInterface) {
        this.keepCopyForRedirection = action.parameters.keepCopyForRedirection;
    }

    toogleKeepOther(action: ActionAdminInfoInterface) {
        this.keepOtherRoleForRedirection = action.parameters.keepOtherRoleForRedirection;
    }

    toogleFilterAbsent(action: ActionAdminInfoInterface) {
        this.filterAbsentUsers = action.parameters.filterAbsentUsers;
    }
