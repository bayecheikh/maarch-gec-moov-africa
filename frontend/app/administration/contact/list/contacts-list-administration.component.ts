...
export class ContactsListAdministrationComponent implements OnInit {
    ...
    displayedColumnsContact: string[] = ['filling', 'confidentiality', 'firstname', 'lastname', 'company', 'formatedAddress', 'actions'];
    ...
    processPostData(data: any) {
        data.contacts.forEach((element: any) => {
            const contactAddress: AddressContactInterface = this.contactService.setContactAddress(element);
            const addressOnlyValues: { [key: string]: string } = this.contactService.getAddressValues(contactAddress);

            element.formatedAddress = this.contactService.formatContactAddress(addressOnlyValues);
        });

        if (!this.functions.empty(data.contacts[0]) && !this.functions.empty(data.contacts[0].filling)) {
            this.displayedColumnsContact = ['select', 'filling', 'confidentiality', 'firstname', 'lastname', 'company', 'formatedAddress', 'actions'];
        } else {
            this.displayedColumnsContact = ['select', 'confidentiality', 'firstname', 'lastname', 'company', 'formatedAddress', 'actions'];
        }
        return data;
    }
    ...
    toggleContact(contact: any) {
        const dialogRef = this.dialog.open(ConfirmComponent, {
            ...
            autoFocus: false,
            disableClose: true,
            data: { title: this.translate.instant('lang.suspend'), msg: this.translate.instant('lang.confirmAction') }
        });
    ...
    isConfidentialContact(contact: AddressContactInterface & { phone: Field, email: Field }): boolean {
        const filteredFields: Field[] = [];

        Object.keys(contact).forEach((key: string) => {
            if (this.contactService.confidentialFields.includes(key)) {
                const field = (contact as any)[key] as Field;
                if (field) filteredFields.push(field);
            }
        });

        return filteredFields.every((item: { confidential: boolean }) => item.confidential);
    }
...
type Field = { confidential: boolean; value?: string | null };
