...
export class ContactResourceComponent implements OnInit {
    ...
    getCustomFields(): Promise<boolean> {
    ...
    loadContactsOfResource(resId: number, mode: string): void {
            ...
            tap((data: any) => {
                ...
                this.contacts.forEach((contact: any, index: number) => {
                    if (contact.type === 'contact') {
                        this.contacts[index] = this.contactService.formatConfidentialFields(contact);
                    }
                });
            }),
    ...
    formatCustomField(data: any): { label: string, value: string }[] {
        const arrCustomFields: { label: string, value: string }[] = [];
