...
export class SignatureBookOldComponent implements OnInit, OnDestroy {
    ...
    loadActions() {
        this.http.get('../rest/resourcesList/users/' + this.userId + '/groups/' + this.groupId + '/baskets/' + this.basketId + '/actions?resId=' + this.resId).pipe(
            tap((data: any) => {
    ...
    processAction() {
        this.http.get(`../rest/resources/${this.resId}?light=true`).pipe(
            tap((data: any) => {
                ...
                const selectedAction = this.signatureBook.actions.filter((action: ActionInterface) => action.id == actionId)[0];
