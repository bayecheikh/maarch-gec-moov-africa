...
export class BasketListComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.subscription2 = this.actionService.catchAction().subscribe(() => {
    ...
    initResultList() {
        ...
        merge(this.sort.sortChange, this.paginator.page, this.filtersChange)
            .pipe(
                ...
                catchError((err: any) => {
                    /**
                     * Display the error message: Basket out of your perimeter
                     * if and only if the error contains the message Basket out of permimeter
                     * and it's the same user logging in.
                     */
                    if (err.error.errors.includes('Basket out of perimeter')) {
                        const url: string = err.url.split('/');
                        if (url[url.indexOf('users') + 1] === this.headerService.user.id.toString()) {
                            this.notify.handleSoftErrors(this.translate.instant('lang.basketOutOfPerimeter'));
                        }
                    } else {
                        this.notify.handleErrors(err);
                    }
    ...
    viewDocument(row: any) {
        this.http.get(`../rest/resources/${row.resId}/content?mode=view`, { responseType: 'blob' }).pipe(
            tap((data: any) => {
                ...
                newWindow.document.write(`<iframe style="width: 100%;height: 100%;margin: 0;padding: 0;" src="${fileURL}" allowfullscreen></iframe>`);
