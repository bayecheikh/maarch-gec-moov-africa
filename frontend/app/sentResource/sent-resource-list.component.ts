...
export class SentResourceListComponent implements OnInit {
            ...
            this.http.get(`../rest/resources/${this.resId}/acknowledgementReceipts?type=ar`).pipe(
                map((data: any) => {
                    data = data.map((item: any) => {
                        ...
                        let name: string;
                        if (!this.functions.empty(item.contact.labelToDisplay)) {
                            name = item.contact.labelToDisplay;
                        ...
                        return {
                            id: item.id,
                            sender: false,
                            recipients: item.format === 'html' ? this.contactService.getRecipientsByCorrespondantTarget([{
                                ...email,
                                type: 'contact'
                            }]) : name,
            ...
            this.http.get(`../rest/resources/${this.resId}/emails?type=email`).pipe(
                map((data: any) => {
                    data.emails = data.emails.map((item: any) => ({
                        ...
                        recipients: this.contactService.getRecipientsByCorrespondantTarget(item.recipients),
        ...
        return new Promise((resolve) => {
            this.http.get(`../rest/resources/${this.resId}/shippings`).pipe(
                map((data: any) => {
                    data = data.map((item: any) => ({
                        id: item.id,
                        sendingId: item.sendingId,
                        sender: item.userLabel,
                        sendMode: item.sendMode ?? '',
                        recipients: item.recipients,
                        recipientId: item.recipients[0].recipientId,
                        creationDate: item.creationDate,
                ...
                tap((data: any) => {
                    data.forEach((element: any) => {
                        element.recipients.forEach((el: any, index: number) => {
                            delete el.recipientId;
                            // convert an object to array to use a filter function
                            el = Object.values(el);
    ...
    openPromptNumericPackage(row: any = { id: null, type: null }) {
        ...
        if (row.canManage || row.id === null) {
            const dialogRef = this.dialog.open(SentNumericPackagePageComponent, {
                panelClass: 'maarch-modal',
                width: '60vw',
                disableClose: true,
                data: { title: title, resId: this.resId, emailId: row.id }
            });
