...
import { catchError, exhaustMap, filter, finalize, map, take, tap } from 'rxjs/operators';
...
export class DocumentViewerComponent implements OnInit, OnDestroy {
    ...
    /**
     * base64 of a document (@format is required!)
     */
    ...
    /**
     * Resource of a document link to attachment (@mode = 'attachment' required!)
     */
    @Input() resIdMaster: number = null;

    /**
     * Can manage a document? (create, delete, update)
     */
    ...
    /**
     * Title of a new tab when open a document in an external tab
     */
    @Input() title: string = '';

    /**
     * To load a specific attachment type in a template list (to create a document)
     */
    ...
    @Input() isFromInternalParaph: boolean = false;

    /**
     * To load a specific attachment type in a template list (to create a document)
     */
    @Input() lastUnannotatedVersionFormat: string = null;
    ...
    constructor(
        ...
        /**
         * The `pdf.worker.min.mjs` file is used instead of `pdf.worker.min.js` starting from
         * version 4.x of `pdfjs-dist` to align with modern JavaScript module standards.
         *
         * The `.mjs` extension indicates that this file is an ES (ECMAScript) module,
         * which is necessary for environments that support modules like modern browsers
         * or bundlers like Webpack.
         *
         * Using `.mjs` allows `pdfjs-dist` to take advantage of native module loading,
         * providing better optimization and compatibility with JavaScript’s module system.
         *
         * It is important to configure the `workerSrc` path to point to this `.mjs` file
         * in projects using `pdfjs-dist` version 4.x and above.
         **/
        ...
        if (!this.functions.empty(this.base64)) {
            ...
            this.http.get('../rest/convertedFile/' + this.tmpFilename).pipe(
                tap((data: any) => {
                    ...
                    this.headerService.base64 = this.file.content;
    ...
    uploadTrigger(fileInput: any) {
        if (fileInput.target.files && fileInput.target.files[0] && this.isExtensionAllowed(fileInput.target.files[0])) {
            ...
            if (this.isNewVersion && this.mode !== 'attachment') {
                ...
                this.triggerEvent.emit('isNewVersionUploaded');
            }
        } else {
            this.loading = false;
        }
    ...
    setNewVersion(fileInput: any, file: any) {
        if (this.canBeConverted(file)) {
            ...
            reader.onload = async (value: any) => {
                ...
                this.headerService.base64 = file.content;
                this.triggerEvent.emit('uploadFile');
    ...
    getBase64Document(buffer: ArrayBuffer) {
        try {
            ...
            const TYPED_ARRAY = new Uint8Array(bufferCopy);
    ...
    convertDocument(file: any) {
        if (this.canBeConverted(file)) {
            ...
            this.noConvertedFound = true;
            this.loading = false;
            this.headerService.base64 = null;
        }
    }
    ...
    resetFileData() {
        ...
        this.headerService.base64 = null;
        this.headerService.resIdMaster = null;
        ...
        return new Promise((resolve) => {
            if (!this.functions.empty(this.file.src)) {
                ...
                this.getFile().pipe(
                    ...
                    exhaustMap((data: any) => this.http.post('../rest/convertedFile', {
    ...
    canBeConverted(file: any): boolean {
        ...
        return this.allowedExtensions.filter(ext => ext.canConvert === true && ext.mimeType === file.type && ext.extension === fileExtension).length > 0;
    }
        ...
        if (allowedExtensions.filter(ext => (ext.mimeType === file.type || (this.functions.empty(ext.mimeType) && this.functions.empty(file.type))) && ext.extension === fileExtension).length === 0) {
            this.dialog.open(AlertComponent, {
                ...
                data: {
                    ...
                    msg: this.translate.instant('lang.file') + ' : <b>' + file.name + '</b>, ' + this.translate.instant('lang.type') + ' : <b>' + file.type + '</b><br/><br/><u>' + this.translate.instant('lang.allowedExtensions') + '</u> : <br/>' + allowedExtensions.map(ext => ext.extension).filter((elem: any, index: any, self: any) => index === self.indexOf(elem)).join(', ')
                }
            });
            return false;
            ...
            this.dialog.open(AlertComponent, {
                ...
                disableClose: true,
                data: {
                    title: this.translate.instant('lang.maxFileSizeReached') + ' ! ',
                    msg: this.translate.instant('lang.maxFileSize') + ' : ' + this.maxFileSizeLabel
                }
            });
            return false;
        } else {
            ...
            this.requestWithLoader(`../rest/attachments/${resId}/content?mode=base64`).subscribe(
                (data: any) => {
                    if (data.encodedDocument) {
                        ...
                        this.loading = false;
                        this.noFile = false;
                        this.headerService.base64 = data.encodedDocument;
                    }
                },
                (err: any) => {
                    if (err.error.errors === 'Document has no file') {
                        ...
                        err.error.errors === 'Converted Document not found' ||
                        err.error.errors === 'Document can not be converted' ||
                        err.error.errors.startsWith('Conversion failed')
                    ) {
                        this.file.contentMode = 'route';
                        this.file.content = `../rest/attachments/${resId}/originalContent?mode=base64`;
                        this.noConvertedFound = true;
            ...
            this.http.get(`../rest/resources/${this.resId}/versionsInformations`).pipe(
                tap((data: any) => {
                    ...
                    if (data.DOC[data.DOC.length - 1] !== undefined) {
                        signedDocVersions = data.SIGN.indexOf(data.DOC[data.DOC.length - 1]) > -1;
                        commentedDocVersions = data.NOTE.indexOf(data.DOC[data.DOC.length - 1]) > -1;
                        mainDocPDFVersions = data.PDF.indexOf(data.DOC[data.DOC.length - 1]) > -1;
                    }

                    let lastUnsignedVersion = -1;
                    if (data.lastUnsignedVersion !== undefined) {
                        lastUnsignedVersion = data.lastUnsignedVersion;
                    }
                    ...
                    this.file.subinfos = {
                        ...
                        lastUnsignedVersion: lastUnsignedVersion
                    };
                }),
    ...
    confirmEditTemplate(templateId: number) {
        let confirmMsg = '';
        const canViewConfidentialInformation: boolean = this.headerService.user.privileges.indexOf('view_confidential_contact_information') > -1;
        ...
        if (!canViewConfidentialInformation) {
            confirmMsg += '<br><br><b>' + this.translate.instant('lang.cannotMergeConfidentialFields') + '</b><br>';
        }
        this.dialogRef = this.dialog.open(ConfirmComponent, {
            panelClass: 'maarch-modal',
            autoFocus: false,
            disableClose: true,
            data: {
                idModal: 'confirmEditTemplate',
                title: this.translate.instant('lang.templateEdition'),
                msg: confirmMsg
            }
        });
    ...
    editResource() {
        ...
        } else if (this.editor.mode === 'java' && this.file.format.toLowerCase() === 'pdf' && !this.isAnnotated) {
            this.notify.error(this.translate.instant('lang.javaEditDenied') + ' <b>PDF</b> ' + this.translate.instant('lang.javaEditDenied2'));
        } else {
            const isAnnotated: boolean = (this.isFromInternalParaph && this.signatureBookService.docsToSign.find((attachment: AttachmentInterface) => attachment.resId === this.resId)?.isAnnotated) || this.isAnnotated;
            const target: string = this.mode === 'attachment' ? 'editAttachment' : 'editMainDocument';
            if (isAnnotated) {
                ...
                dialogRef.afterClosed().pipe(
                    filter((data: string) => data === 'ok'),
                    tap(() => this.editAttachmentOrMainDocument(target)),
                    catchError((err: any) => {
                        this.notify.handleSoftErrors(err);
                        return of(false);
                    })
                ...
                this.editAttachmentOrMainDocument(target);
            }
        }
    }

    editAttachmentOrMainDocument(target: string): void {
        this[target]();
    }
    ...
    editAttachment() {
        ...
        this.triggerEvent.emit('isFileEdited');
    }
    ...
    closeEditor(isSaveEvent: boolean = false) {
        this.templateListForm.reset();
        this.editInProgress = false;
        this.isDocModified = false;
        if (!isSaveEvent) {
            this.templateId = null;
            this.triggerEvent.emit('isEditorClosed');
            this.file.src = this.base64ToArrayBuffer(this.headerService.base64);
        }
    }
            ...
            this.getFile().pipe(
                map((data: any) => {
                    const formatdatas = {
                        ...
                        format: this.lastUnannotatedVersionFormat ?? data.format,
                        resId: this.resId
                    };
                    return formatdatas;
                }),
    ...
    saveTmpDocument() {
        ...
        return new Promise((resolve) => {
            this.getFile().pipe(
                ...
                exhaustMap((data) => this.http.post('../rest/convertedFile/encodedFile', {
                    format: this.lastUnannotatedVersionFormat ?? data.format,
                    encodedFile: data.content
                })),
                tap((data: any) => {
    ...
    unsignMainDocument() {
        this.dialogRef = this.dialog.open(ConfirmComponent, {
            ...
            autoFocus: false,
            disableClose: true,
            data: { title: this.translate.instant('lang.UNSIGN'), msg: this.translate.instant('lang.confirmAction') }
        });
    ...
    canDownloadProof(): boolean {
        return this.resId !== null &&
            this.file?.subinfos?.signedDocVersions &&
            (
                (
                    !this.functions.empty(this.externalId?.internalParapheur) &&
                    this.signatureBookService.config.isNewInternalParaph &&
                    this.privilegeService.hasCurrentUserPrivilege('download_proof_file')
                )
                ||
                !this.functions.empty(this.externalId[this.externalSignatoryBook.signatoryBookEnabled])
            )
    }

    async downloadProof(data: { resId: number; chrono: string; externalId: Record<string, any> }): Promise<void> {
        this.downloadingProof = true;

        try {
            const externalId = data.externalId[this.externalSignatoryBook.signatoryBookEnabled];

            if (this.functions.empty(externalId)) {
                await this.signatureBookService.downloadProof(data, false);
            } else {
                await this.externalSignatoryBook.downloadProof(externalId);
            }
        } finally {
