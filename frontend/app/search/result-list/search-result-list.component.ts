...
import { EscapeHtmlPipe } from '@plugins/escapeHtml.pipe';
...
@Component({
    ...
    providers: [HighlightPipe, EscapeHtmlPipe]
})
export class SearchResultListComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        public escapeHtmlPipe: EscapeHtmlPipe,
    ...
    getSubject(subject: string): string {
        const criteria: string[] = this.criteria ? Object.keys(this.criteria) : [];
        return (criteria.indexOf('meta') > -1 || criteria.indexOf('subject') > -1) ? subject : this.escapeHtmlPipe.transform(subject);
    }
}
