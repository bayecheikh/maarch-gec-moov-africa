    ...
    executeAction() {
        ...
        if (this.canAddCopies) {
            const cc: {
                ...
                type: string;
                email?: string;
            }[] = this.appMailEditor.mapEmailsByCorrespondentTarget(this.appMailEditor.getCopies());

            const cci: {
                id?: number;
                type?: string;
                email?: string;
            }[] = this.appMailEditor.mapEmailsByCorrespondentTarget(this.appMailEditor.getInvisibleCopies());

            data = { ...data, sender: this.appMailEditor.getSender(), cc: cc, cci: cci };
        }
        this.http.put(this.data.processActionRoute, {
            resources: this.realResSelected,
