...
export class RedirectActionComponent implements OnInit {
    ...
    filterAbsentUsers: boolean = false;
    ...
    canBackToRedirect: boolean = true;
    ...
    async ngOnInit(): Promise<void> {
        ...
        if (this.actionKeyword === 'autoRedirectToUser') {
            ...
            this.loading = false;
            this.canBackToRedirect = false;
            if (this.entities.filter((entity: any) => entity.allowed).length > 0) {
                this.canBackToRedirect = true;
            }
        ...
        return new Promise((resolve) => {
            this.http.get(`../rest/resourcesList/users/${this.data.userId}/groups/${this.data.groupId}/baskets/${this.data.basketId}/actions/${this.data.action.id}/getRedirect`).pipe(
                tap((data: any) => {
                    ...
                    this.injectDatasParam.filterAbsentUsers = data.filterAbsentUsers ?? false;
                    resolve(true);
                }),
                catchError((err: any) => {
    ...
    changeDest(event: any) {
        ...
        if (this.data.resIds.length === 1) {
            ...
            this.http.get('../rest/resources/' + this.data.resIds[0] + '/users/' + user.id + '/isDestinationChanging').pipe(
                tap((data: any) => {
    ...
    onCancel(): void {
        if (this.redirectMode === '' || !this.canBackToRedirect) {
