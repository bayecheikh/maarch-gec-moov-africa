...
} from './visa-reject-back-to-previous-action/reject-visa-back-to-previous-action.component';
...
} from './update-acknowledgement-send-date-action/update-acknowledgement-send-date-action.component';
import {
    CreateAcknowledgementReceiptActionComponent
} from './create-acknowledgement-receipt-action/create-acknowledgement-receipt-action.component';
import {
    UpdateDepartureDateActionComponent
} from './update-departure-date-action/update-departure-date-action.component';
import {
    DisabledBasketPersistenceActionComponent
} from './disabled-basket-persistence-action/disabled-basket-persistence-action.component';
import {
    EnabledBasketPersistenceActionComponent
} from './enabled-basket-persistence-action/enabled-basket-persistence-action.component';
...
} from './send-external-signatory-book-action/send-external-signatory-book-action.component';
import {
    SendExternalNoteBookActionComponent
} from './send-external-note-book-action/send-external-note-book-action.component';
...
} from './redirect-initiator-entity-action/redirect-initiator-entity-action.component';
import {
    closeMailWithAttachmentsOrNotesActionComponent
} from './close-mail-with-attachments-or-notes-action/close-mail-with-attachments-or-notes-action.component';
...
} from './visa-continue-circuit-action/continue-visa-circuit-action.component';
...
} from './avis-continue-circuit-action/continue-avis-circuit-action.component';
...
} from './save-and-print-registered-mail-action/save-and-print-registered-mail-action.component';
import {
    SaveAndIndexRegisteredMailActionComponent
} from './save-and-index-registered-mail-action/save-and-index-registered-mail-action.component';
import {
    PrintRegisteredMailActionComponent
} from './print-registered-mail-action/print-registered-mail-action.component';
...
} from './check-reply-record-management-action/check-reply-record-management.component';
...
} from './check-acknowledgment-record-management-action/check-acknowledgment-record-management.component';
import { FiltersListService } from '@service/filtersList.service';
import { SessionStorageService } from '@service/session-storage.service';
import { ActionInterface, DatasActionSendInterface, MessageAction, MessageActionInterface } from '@models/actions.model';
import { SignatureBookService } from '@appRoot/signatureBook/signature-book.service';
import {
    ContinueVisaCircuitActionNewSbComponent
} from './visa-continue-circuit-action/new-signature-book/continue-visa-circuit-action-new-sb.component';
import { MatDialogConfig } from '@angular/material/dialog';
import { ComponentType } from '@angular/cdk/portal';
import { AddUserEntityAsCopyActionComponent } from './add-user-entity-as-copy/add-user-entity-as-copy.component';
...
export class ActionsService implements OnDestroy {
    ...
    currentAction: ActionInterface = null;
    ...
    getActions(userId: number, groupId: number, basketId: number, resId: number): Observable<ActionInterface[]> {
        return this.http
            .get<{ actions: ActionInterface[] }>(
            `../rest/resourcesList/users/${userId}/groups/${groupId}/baskets/${basketId}/actions?resId=${resId}`
        )
            .pipe(
    ...
    setActionInformations(action: ActionInterface, userId: number, groupId: number, basketId: number, resIds: number[]) {
    ...
    launchIndexingAction(action: ActionInterface, userId: number, groupId: number, datas: any) {
    ...
    async launchAction(action: ActionInterface, userId: number, groupId: number, basketId: number, resIds: number[], datas: any, lockRes: boolean = true) {
    ...
    getDefaultAction(): any {
        const objToSend: {
            showToggle: boolean, inLocalStorage: boolean, canGoToNextRes: boolean, inSignatureBook?: boolean
        } = {
            showToggle: false,
            inLocalStorage: false,
            canGoToNextRes: false,
            inSignatureBook: this.currentAction?.parameters?.inSignatureBook ?? false
        };
        if (!this.functions.empty(this.currentResourceInformations.canGoToNextRes)) {
            ...
            if (this.currentResourceInformations.canGoToNextRes === true) {
                ...
                this.sessionStorage.clearAllById({
    ...
    setDatasActionToSend() {
        const datas: DatasActionSendInterface = {
    ...
    endAction(resIds: any) {
        ...
        this.emitActionWithData({ id: 'fetchData', data: resIds });
    ...
    addUserEntityAsCopyAction() {
        const dialogRef = this.dialog.open(AddUserEntityAsCopyActionComponent, {
            panelClass: 'maarch-modal',
            autoFocus: false,
            disableClose: true,
            data: this.setDatasActionToSend()
