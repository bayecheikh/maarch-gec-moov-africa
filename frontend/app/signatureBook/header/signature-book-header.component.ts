...
export class SignatureBookHeaderComponent {
    ...
    async toggleCurrentResource(state: boolean) {
        await this.signatureBookService.toggleSelection(state, this.userId, this.groupId, this.basketId, this.resId);
        this.signatureBookService.isCurrentResourceSelected = state;
    }
    ...
    backToBasket(): void {
        ...
        this.router.navigate([path]);
        this.signatureBookService.canCheckInconsistentPlugin = true;
    }
