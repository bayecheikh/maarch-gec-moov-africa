...
export class ResourcesListComponent implements AfterViewInit, OnInit {
    ...
    constructor(
        ...
        this.actionService.catchActionWithData().subscribe((res: MessageActionInterface) => {
            if (res.id === 'fetchData') {
                this.loading = true;
                this.resources = this.resources.filter((resource: ResourcesList) => res.data.indexOf(resource.resId) === -1);
                this.signatureBookService.resourcesListIds = this.signatureBookService.resourcesListIds.filter((id: number) => res.data.indexOf(id) === -1);
                this.signatureBookService.selectedMailsCount = 0;
                this.loading = false;
            }
        });
    }
    ...
    appendData(data: ResourcesList[], mode: 'before' | 'after' = 'after') {
        data.forEach((resource) => {
            resource.selected = !!(this.signatureBookService.isCurrentResourceSelected && this.resId);
        });
    ...
    goToResource(resId: number): void {
        ...
        // Call the action service to navigate to the resource
        ...
        this.actionsService
            ...
            .subscribe((resourcesToProcess: number[]) => {
                ...
                if (resourcesToProcess.indexOf(resId) > -1) {
                    ...
                    // Notify the user that the resource is locked
    ...
    calculateContainerHeight(): number {
        ...
        setTimeout(() => {
             ...
             * container and call this line there, but it may require a library to detect the resize event.
             * */
