...
} from '../../administration/registered-mail/issuing-site/indexing/issuing-site-input.component';
import {
    RegisteredMailRecipientInputComponent
} from '../../administration/registered-mail/indexing/recipient-input.component';
...
} from '@appRoot/administration/indexingModel/valuesSelector/values-selector.component';
...
export class IndexingFormComponent implements OnInit {
    ...
    availableFields: any[] = [
        ...
        },
        {
            ...
            type: 'radio',
            default_value: null,
            values: [{ 'id': true, 'label': this.translate.instant('lang.yes') }, {
                'id': false,
                'label': this.translate.instant('lang.no')
            }],
            enabled: true,
        },
        {
        ...
        },
        {
            ...
            type: 'select',
            default_value: null,
            values: [{ 'id': '2D', 'label': this.translate.instant('lang.registeredMail_2D') }, {
                'id': '2C',
                'label': this.translate.instant('lang.registeredMail_2C')
            }, { 'id': 'RW', 'label': this.translate.instant('lang.registeredMail_RW') }],
            enabled: true,
        },
        {
    ...
    removeItem(arrTarget: string, item: any, index: number) {
        this.dialogRef = this.dialog.open(ConfirmComponent, {
            ...
            disableClose: true,
            data: {
                title: this.translate.instant('lang.indexingModelModification'),
                msg: this.translate.instant('lang.updateIndexingFieldWarning')
            }
        });
    ...
    async fillWithLad(ladResult: any) {
        ...
        await Promise.all(this.fieldCategories.map(async (element) => {
            await Promise.all(this['indexingModels_' + element].map(async (elem: any) => {
                if (elem.type === 'date' && !this.functions.empty(ladResult[elem.identifier])) {
                    this.arrFormControl[elem.identifier].setValue(new Date(ladResult[elem.identifier]));
                } else if ((elem.type === 'string' || elem.type === 'integer') && !this.functions.empty(ladResult[elem.identifier])) {
                    this.arrFormControl[elem.identifier].setValue(ladResult[elem.identifier]);
                } else if ((elem.type === 'select' || elem.type === 'radio') && !this.functions.empty(ladResult[elem.identifier])) {
                    const valueExists: boolean = elem.values.some((option: any) => option.id === ladResult[elem.identifier]);
                    if (valueExists) {
                        this.arrFormControl[elem.identifier].setValue(ladResult[elem.identifier]);
                    }
    ...
    async loadForm(indexModelId: number, saveResourceState: boolean = true) {
        ...
        if (!this.adminMode) {
            this.arrFormControl['mail­tracking'] = new UntypedFormControl({
                value: '',
    ...
    initValidator(field: any) {
        ...
        this.arrFormControl[field.identifier] = new UntypedFormControl({
            ...
            if (!this.adminMode && this.arrFormControl['processLimitDate'] !== undefined) {
                this.http.get('../rest/indexing/processLimitDate', { params: objToSend }).pipe(
                    tap((data: any) => {
                        ...
                        if (this.functions.empty(limitDate) || this.functions.empty(this.arrFormControl['priority'])) {
                            resolve(true);
                            return;
                        }
                    }),
                    filter((data: { processLimitDate: string }) => data.processLimitDate !== null),
    ...
    transferRegisteredMailInput(event: CdkDragDrop<string[]>) {
        ...
        if (event.item.data.identifier !== 'registeredMail_type') {
            const obj = event.previousContainer.data.map((item: any, indexItem: number) => ({
                index: indexItem,
                identifier: item.identifier
            })).filter((item: any) => item.identifier === 'registeredMail_type')[0];
