...
export class AttachmentsListComponent implements OnInit {
    ...
    @Input() hideIntegrationActions: boolean = false;
    @Input() isModal: boolean = false;
    @Input() isFromInternalParaph = false;
    ...
    @Output() attachmentsModificationSb = new EventEmitter<{
        resId: number,
    ...
    async ngOnInit(): Promise<void> {
        ...
        this.route.params.subscribe(async (param: any) => {
            if (this.resId !== null) {
                ...
                if (this.isFromInternalParaph) {
                    this.groupId = param['groupId'];
                    this.basketId = param['basketId'];
                    queryParams = `groupId=${this.groupId}&basketId=${this.basketId}`;
                }

                this.http.get(`../rest/resources/${this.resId}/attachments?${queryParams}`).pipe(
                    tap((data: any) => {
                        ...
                        this.groupId = param['groupSerialId'] ?? param['groupId'];
    ...
    showAttachment(attachment: any) {
            ...
            data:
                {
                    ...
                    editMode: (attachment.canUpdate && this.canModify),
                    isFromInternalParaph: this.isFromInternalParaph,
        ...
        this.dialogRef.afterClosed().pipe(
            ...
            tap(() => {
                ...
                if (this.isFromInternalParaph) {
                    const isDocModified: boolean = this.dialogRef.componentInstance.appAttachmentViewer.isDocModified;
                    const isNewVersion: boolean = this.dialogRef.componentInstance.newVersion;
                    const originalId: number = this.dialogRef.componentInstance.resId
                    this.attachmentsModificationSb.emit({
                        resId: attachment.resId,
                        isDocModified: isDocModified,
    ...
    createAttachment() {
        this.headerService.setResIdMaster(this.resId);

        this.dialogRef = this.dialog.open(AttachmentCreateComponent, {
            disableClose: true,
            panelClass: 'attachment-modal-container',
            height: '90vh',
            width: this.appService.getViewMode() ? '99vw' : '90vw',
            ...
            tap(() => {
                ...
                if (this.isFromInternalParaph) {
                    const attachments: {
                        ...
                        type: string,
                        signable: boolean
                    }[] = this.dialogRef.componentInstance.attachmentDetails;
                    this.attachmentsCreationSb.emit(attachments)
                }
            }),
            catchError((err: any) => {
                this.notify.handleErrors(err);
    ...
    deleteAttachment(attachment: any) {
        ...
        this.dialogRef.afterClosed().pipe(
            ...
            tap(() => {
                this.attachmentsClone = JSON.parse(JSON.stringify(this.attachments));
                const cloneAttachment = this.attachmentsClone.find((a: any) => a.resId === attachment.resId);
                cloneAttachment && (cloneAttachment.signable = false);
                this.afterActionAttachment.emit({ id: 'setInSignatureBook', attachments: this.attachmentsClone });

                this.loadAttachments(this.resId);
                this.notify.success(this.translate.instant('lang.attachmentDeleted'));
                if (this.isFromInternalParaph) {
                    this.attachmentsDeletionSb.emit(attachment.resId);
                }
            }),
            catchError((err: any) => {
                this.notify.handleErrors(err);
                ...
                tap((data: any) => {
                    Object.keys(data.attachmentsTypes).forEach((type: any) => {
                        this.attachmentTypes.push({
                            ...
                            signedByDefault: data.attachmentsTypes[type].signedByDefault
                        });
                    });
    ...
    setTaget(target: string): void {
        ...
        const attachmentsWithValidStatus: any[] = this.attachmentsClone.filter((attachment: any) => (attachment.status === 'A_TRA' || attachment.status === 'SEND_MASS'));
    ...
    canDownloadProof(attachment: {
        status: string,
        type: string,
        external_id: { internalParapheur: number }
    }): boolean {
        const attachmentType: {
            signedByDefault: boolean
        } = this.attachmentTypes.find((type: any) => type.typeId === attachment.type);
        if (attachmentType.signedByDefault) {
            return false;
        }

        const internalParaphId: number = attachment?.external_id?.internalParapheur;
        return attachment.status === 'SIGN' &&
            (
                (!this.functions.empty(internalParaphId) &&
                    this.signatureBookService.config.isNewInternalParaph &&
                    this.privilegeService.hasCurrentUserPrivilege('download_proof_file')
                ) ||
                (!this.functions.empty(attachment.external_id[this.externalSignatoryBook.signatoryBookEnabled]))
            );
    }

    async downloadProof(data: { resId: number; chrono: string; external_id: Record<string, any> }): Promise<void> {
        this.downloadingProof = true;

        try {
            const externalId = data.external_id[this.externalSignatoryBook.signatoryBookEnabled];

            if (this.functions.empty(externalId)) {
                await this.signatureBookService.downloadProof(data, true);
            } else {
