...
export class AttachmentCreateComponent implements OnInit, OnDestroy {
    ...
    isSavingAttachment: boolean = false;

    defaultType: any = null;

    attachmentDetails: { resId: number, type: string, signable: boolean }[] = [];
                ...
                tap(async (data: any) => {
                    ...
                    if (data.categoryId === 'outgoing') {
                        if (!this.functions.empty(data.recipients) && data.recipients.length > 0) {
                            ...
                            contact = !this.functions.empty(this.resourceContacts) ? [{
                                id: this.resourceContacts[0].id,
                                type: this.resourceContacts[0].type
                            }] : '';
                        }
                    } else {
                    ...
                    this.attachments.push({
                        ...
                        templateId: new UntypedFormControl({ value: null, disabled: false })
                    });
                    setTimeout(() => {
    ...
    formatAttachments() {
        ...
        this.attachments.forEach((element) => {
            formattedAttachments.push({
                ...
                templateId: element.templateId.value,
                inSignatureBook: this.attachmentsTypes.find((type) => type.id === element.type.value).inSignatureBook
            });
        });
    ...
    onSubmit(mode: string = 'default') {
        ...
        const templateId: number = this.appDocumentViewer.toArray()[this.indexTab].templateId;
        this.appDocumentViewer.toArray()[this.indexTab].getFile().pipe(
            ...
            tap((data) => {
                ...
                this.attachments[this.indexTab].templateId.setValue(templateId);
            }),
            tap(async () => {
                if (this.isValid()) {
                    let resId: any = null;
                ...
                tap((data: any) => {
                    if (this.data.isFromInternalParaph) {
                        this.attachmentDetails.push({
                            ...
                            signable: this.attachmentsTypes.filter((type: {
                                typeId: string
                            }) => type.typeId === attachment.type)[0].signable
                        });
                    }
                    resolve(data.id);
                }),
    ...
    isDocLoading() {
        let state = false;
        this.appDocumentViewer.toArray().forEach((app: DocumentViewerComponent) => {
    ...
    newPj() {
        this.attachments.push({
            ...
            recipient: new UntypedFormControl({
                value: !this.functions.empty(this.resourceContacts[this.attachments.length]) ? [{
                    id: this.resourceContacts[this.attachments.length].id,
                    type: this.resourceContacts[this.attachments.length].type
                }] : null, disabled: false
            }),
    ...
    removePj(i: number) {
        const dialogRef = this.dialog.open(ConfirmComponent, {
            ...
            disableClose: true,
            data: {
                title: this.translate.instant('lang.delete') + ' : ' + this.translate.instant('lang.attachmentShort') + ' n°' + (i + 1),
