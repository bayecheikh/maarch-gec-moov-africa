...
export class AttachmentPageComponent implements OnInit, OnDestroy {
    ...
    newVersionWithTemplate: boolean = false;
    ...
    ngOnDestroy() {
        if (!['process/users', '/resources'].some(path => this.router.url.includes(path))) {
            this.headerService.resetBase64AndResId();
        }
    }
    ...
    loadAttachment() {
        return new Promise((resolve) => {
            ...
            if (this.data.isFromInternalParaph) {
                queryParams = `groupId=${this.data.groupId}&basketId=${this.data.basketId}`;
            }
            this.http.get(`../rest/attachments/${this.data.resId}?${queryParams}`).pipe(
                tap((data: any) => {
                    ...
                    this.attachment = {
                        ...
                        typistLabel: new UntypedFormControl({
                            value: data.typistLabel,
                            disabled: true
                        }, [Validators.required]),
                        ...
                        resIdMaster: new UntypedFormControl({
                        ...
                        validationDate: new UntypedFormControl({
                            value: data.validationDate !== null ? new Date(data.validationDate) : null,
                            disabled: !this.editMode
                        }),
                        signedResponse: new UntypedFormControl({ value: data.signedResponse, disabled: false }),
                        encodedFile: new UntypedFormControl({
                            value: '_CURRENT_FILE',
                            disabled: !this.editMode
                        }, [Validators.required]),
                    ...
                    this.headerService.setResIdMaster(this.attachment.resIdMaster.value);
    ...
    createNewVersion(mode: string = 'default') {
        ...
        this.appAttachmentViewer.getFile().pipe(
            ...
            exhaustMap(() => this.http.post(`../rest/attachments?withTemplate=${this.newVersionWithTemplate}`, this.getAttachmentValues(true, mode))),
            tap(async (data: any) => {
                this.resId = data.id;
    ...
    setDatasViewer(ev: any) {
        ...
        if (this.newVersion && ['isFileEdited', 'isNewVersionUploaded', 'isEditorClosed', 'cleanFile'].indexOf(ev) > -1) {
            this.newVersionWithTemplate = ev === 'isFileEdited';
        }
    }
    ...
    setNewVersion() {
        if (!this.newVersion) {
            const dialogRef = this.dialog.open(ConfirmComponent, {
                ...
                disableClose: true,
                data: {
                    title: this.translate.instant('lang.createNewVersion'),
    ...
    deleteSignedVersion() {
        const dialogRef = this.dialog.open(ConfirmComponent, {
            ...
            disableClose: true,
            data: {
                title: this.translate.instant('lang.deleteSignedVersion'),
                msg: this.translate.instant('lang.confirmAction')
            }
        });
    ...
    closeModal() {
        ...
        if (this.appAttachmentViewer.isEditingTemplate()) {
            const dialogRef = this.dialog.open(ConfirmComponent, {
                ...
                disableClose: true,
                data: {
                    title: this.translate.instant('lang.close'),
                    msg: this.translate.instant('lang.editingDocumentMsg')
                }
            });
