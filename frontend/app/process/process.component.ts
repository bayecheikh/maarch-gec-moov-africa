...
export class ProcessComponent implements OnInit, OnDestroy {
    ...
    selectedAction: ActionInterface = {
        id: 0,
        label: '',
        component: '',
        default: false,
    ...
    processTool: ProcessToolInterface[] = [
        {
            id: 'dashboard',
            icon: 'fas fa-columns',
    ...
    senderLightInfo: { displayName: string, filling?: string } = { displayName: null, filling: null };
    ...
    lastUnannotatedVersionFormat: string = null;
    ...
    checkAccesDocument(resId: number): Promise<boolean> {
    ...
    async initProcessPage(params: any): Promise<void> {
    ...
    async initDetailPage(params: any): Promise<void> {
    ...
    loadResource(redirectDefautlTool: boolean = true): void {
        if (!this.logoutTrigger) {
            this.http.get(`../rest/resources/${this.currentResourceInformations.resId}?light=true`).pipe(
                tap((data: any) => {
                    ...
                    this.isAnnotated = data.isAnnotated ?? false;
                    this.lastUnannotatedVersionFormat = data.lastUnannotatedVersionFormat;
            ...
            this.http.get(`../rest/resources/${this.currentResourceInformations.resId}/users/${this.currentUserId}/groups/${this.currentGroupId}/baskets/${this.currentBasketId}/processingData`).pipe(
                tap((data: any) => {
                    if (data.listEventData !== null) {
                        ...
                        this.currentResourceInformations = {
                            ...this.currentResourceInformations,
                            canGoToNextRes: this.canGoToNextRes
                        };
                    }
                }),
                catchError((err: any) => {
    ...
    loadAvaibleIntegrations(integrationsData: { inSignatureBook: boolean }): void {
    ...
    toggleIntegration(integrationId: string): void {
        this.http.put('../rest/resourcesList/integrations', {
            resources: [this.currentResourceInformations.resId],
            integrations: { [integrationId]: !this.currentResourceInformations.integrations[integrationId] }
        }).pipe(
            tap(() => {
    ...
    loadSenders(): void {
        if (this.currentResourceInformations.senders === undefined || this.currentResourceInformations.senders.length === 0) {
                    ...
                    tap((data: any) => {
                        ...
                        if (this.empty(data.firstname) && this.empty(data.lastname)) {
                            if (!this.functions.empty(data.fillingRate)) {
                                this.senderLightInfo = {
                                    'displayName': data.company,
                                    'filling': this.contactService.getFillingColor(data.fillingRate.thresholdLevel)
                                };
                            } else {
                            ...
                            if (!this.functions.empty(data.fillingRate)) {
                                this.senderLightInfo = {
                                    'displayName': arrInfo.filter(info => info !== '').join(' '),
                                    'filling': this.contactService.getFillingColor(data.fillingRate.thresholdLevel)
                                };
                            } else {
            ...
            this.hasContact = true;
            this.senderLightInfo = {
                'displayName': this.currentResourceInformations.senders.length + ' ' + this.translate.instant('lang.senders'),
                'filling': null
            };
        }
    }
    ...
    loadRecipients(): void {
        if (this.currentResourceInformations.recipients === undefined || this.currentResourceInformations.recipients.length === 0) {
            ...
            this.hasContact = true;
            this.senderLightInfo = {
                'displayName': this.currentResourceInformations.recipients.length + ' ' + this.translate.instant('lang.recipients'),
                'filling': null
            };
        }
    }
    ...
    async processAction(): Promise<void> {
        if (this.indexingForm.isValidForm()) {
            this.actionService.setLoading(true);
            if (this.isToolModified()) {
                const dialogRef = this.openConfirmModification();
                dialogRef.afterClosed().pipe(
                    tap((data: string) => {
    ...
    showActionInCurrentCategory(action: any): boolean {
    ...
    openTechnicalInfo(): void {
        this.dialog.open(TechnicalInformationComponent, {
            panelClass: 'maarch-modal',
            autoFocus: false,
            data: { resId: this.currentResourceInformations.resId }
        });
    }
    ...
    isModalOpen(tool = this.currentTool): boolean {
    ...
    async saveModificationBeforeClose(): Promise<void> {
        if (this.isToolModified() && !this.isModalOpen()) {
            await this.saveTool();
        }
    ...
    refreshBadge(nbRres: any, id: string): void {
        this.processTool.filter(tool => tool.id === id)[0].count = nbRres;
    }

    openContact(): void {
        if (this.hasContact) {
            this.dialog.open(ContactResourceModalComponent, {
                panelClass: 'maarch-modal',
                data: {
                    title: `${this.currentResourceInformations.chrono} - ${this.currentResourceInformations.subject}`,
                    mode: this.currentResourceInformations.categoryId !== 'outgoing' ? 'senders' : 'recipients',
    ...
    saveListinstance(): void {
        this.appDiffusionsList.saveListinstance();
    }

    saveVisaWorkflow(): void {
    ...
    isToolModified(): boolean {
        if (this.currentTool === 'info' && this.indexingForm !== undefined && this.indexingForm.isResourceModified()) {
            ...
            return true;
        } else if (this.currentTool === 'visaCircuit' && this.appVisaWorkflow !== undefined && this.appVisaWorkflow.isModified() && !this.appVisaWorkflow.workflowParametersNotValid()) {
            return true;
    ...
    refreshTool(): void {
        const tmpTool: string = this.currentTool;
        this.currentTool = '';
        setTimeout(() => {
    ...
    toggleFreezing(): void {
        ...
        this.http.put('../rest/archival/freezeRetentionRule', {
            resources: [this.currentResourceInformations.resId],
            freeze: this.resourceFreezed
        }).pipe(
            tap(() => {
    ...
    toggleBinding(value): void {
        this.resourceBinded = value;
        this.http.put('../rest/archival/binding', {
            resources: [this.currentResourceInformations.resId],
            binding: value
        }).pipe(
            tap(() => {
    ...
    openPrintedFolderPrompt(): void {
        this.dialog.open(PrintedFolderModalComponent, {
            panelClass: 'maarch-modal',
            data: { resId: [this.currentResourceInformations.resId], multiple: false }
        });
    }
...
interface ProcessToolInterface {
    id: string;
    icon: string;
    label: string;
    editMode?: boolean;
