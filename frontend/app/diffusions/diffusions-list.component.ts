...
export class DiffusionsListComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        ...
        if (this.resId !== null && this.resId != 0 && this.target !== 'redirect') {
            ...
            this.loadListModel(this.entityId, this.selfDest);
    ...
    async loadListModel(entityId: number, destCurrentUser: boolean = false) {
        this.loading = true;
    ...
    changeUserRole(user: any, oldRole: any, newRole: any) {
        ...
        const indexFound: number = this.diffList[oldRole.id].items.map((item: any) => item.itemSerialId).indexOf(user.itemSerialId);
