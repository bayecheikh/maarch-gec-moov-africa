...
import { ActionHistoryInterface } from '@models/actions.model';
    ...
    initFilterListHistory() {
        ...
        if (this.filterList === null) {
            ...
            this.http.get(this.filterListUrl).pipe(
                map((data: any) => {
                    const deletedActions = data.actions.filter((action: ActionHistoryInterface) => action.label === null).map((action: ActionHistoryInterface) => action.id);
                    const deletedUser = data.users.filter((user: any) => user.label === null).map((user: any) => user.login);

                    data.actions = data.actions.filter((action: ActionHistoryInterface) => action.label !== null);
