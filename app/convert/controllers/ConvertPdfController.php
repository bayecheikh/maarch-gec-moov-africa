...
class ConvertPdfController
{
    ...
    public static function convertInPdf(array $aArgs): array
    {
        ...
        if (strtolower($extension) == 'html' || strtolower($extension) == 'htm') {
            ...
            $isLegacyConversionEnable = (new Environment())->isLegacyConversionEnabled();

            if ($isLegacyConversionEnable) {
                $command = "timeout 30 unoconv -f pdf " . escapeshellarg($aArgs['fullFilename']);
            } else {
                //The soffice command is LibreOffice, trust.
                $command = "timeout 30 soffice --headless --convert-to pdf --outdir $tmpPath " .
                    escapeshellarg($aArgs['fullFilename']);
            }
    ...
    public static function tmpConvert(array $aArgs): array
    {
        ...
        if (!file_exists($tmpPath . $docInfo["filename"] . '.pdf')) {
            return [
                'errors' => '[ConvertPdf] ' . _CONVERSION_FAILED,
                'details' => implode(" ", $convertedFile['output'])
            ];
        } else {
    ...
    public static function convert(array $aArgs): array
    {
        ...
        if (strtolower($docInfo["extension"]) != 'pdf') {
            ...
            if (!file_exists($tmpPath . $fileNameOnTmp . '.pdf')) {
                return [
                    'errors' => _CONVERSION_FAILED,
                    'details' => $convertedFile['output']
                ];
            }
        }
    ...
    public static function getConvertedPdfById(array $args): mixed
    {
        ...
        if ($args['collId'] == 'letterbox_coll') {
            ...
            $convertedDocument = AdrModel::getDocuments([
                'select'  => ['id', 'docserver_id', 'path', 'filename', 'fingerprint', 'type'],
     ...
     * @return void
     */
    public static function addBom(string $filePath): void
    {
    ...
    public function convertedFile(Request $request, Response $response): Response
    {
        ...
        if (strtolower($ext) == 'pdf' && strtolower($mimeType) == 'application/pdf') {
            ...
            $conversion = ConvertPdfController::convertFromEncodedResource([
                'encodedResource' => $body['base64'],
                'context'         => $body['context'] ?? null,
                'extension'       => $ext
            ]);
            if (empty($conversion['errors'])) {
                return $response->withJson($conversion);
            } else {
                return $response->withStatus(400)->withJson($conversion);
            }
        }
    }
    ...
    public function getConvertedFileByFilename(Request $request, Response $response, array $args): Response
    {
        ...
        if (!empty($queryParams['convert'])) {
            if (ConvertPdfController::canConvert(['extension' => $extension])) {
                ...
                if (!empty($conversion['errors'])) {
                    $encodedFiles['convertedResourceErrors'] = $conversion['errors'];
                } else {
                    $encodedFiles['encodedConvertedResource'] = $conversion['encodedResource'];
                }
            }
        }
    ...
    public function getConvertedFileFromEncodedFile(Request $request, Response $response): Response
    {
        ...
        if (!empty($conversion['errors'])) {
            return $response->withStatus(400)->withJson(['errors' => $conversion['errors']]);
        }
