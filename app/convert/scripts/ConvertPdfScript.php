...
try {
    ...
    echo "[ConvertPdfScript][Exception] {$e->getMessage()}";
}
// phpcs:enable
...
class ConvertPdfScript
{
     ...
     * @return array
     */
    public static function initialize(array $args): array
    {
        $customId = null;
        $coreUrl = '';
        ...
        if (strtolower($docInfo['extension']) != 'pdf') {
            ...
            if (!$converted) {
                ...
                if ($isLegacyConversionEnable) {
                    ...
                    $command = "soffice --headless --convert-to pdf --outdir $tmpPath " . escapeshellarg($fullFilename);
                }
    ...
    {
        $args = ConvertPdfScript::initialize($args);
