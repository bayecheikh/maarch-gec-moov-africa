...
class DoctypeController
{
    ...
    {
        if (!Validator::intVal()->validate($args['id']) || !Validator::notEmpty()->validate($args['id'])) {
            return $response->withStatus(500)->withJson(['errors' => 'wrong format for id']);
        }

        $obj['doctype'] = DoctypeModel::getById(['id' => $args['id']]);
    ...
    public function delete(Request $request, Response $response, array $args): Response
    {
        ...
        $doctypes = null;
        if ($count[0]['count'] == 0) {
            DoctypeController::deleteAllDoctypeData(['type_id' => $args['id']]);
            $deleted = 0;
            $doctypeTree = FirstLevelController::getTreeFunction();
        } else {
     ...
     * @throws Exception
     */
    public function deleteRedirect(Request $request, Response $response, array $args): Response
    {
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'admin_architecture', 'userId' => $GLOBALS['id']])) {
            return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        }
     ...
     * @throws Exception
     */
    protected function deleteAllDoctypeData(array $args = []): void
    {
        $doctypeInfo = DoctypeModel::getById(['id' => $args['type_id']]);
        if (!empty($doctypeInfo)) {
            DoctypeModel::delete(['type_id' => $args['type_id']]);
     ...
     * @throws Exception
     */
    protected static function control(array $args, string $mode): array
    {
        $errors = [];

        if ($mode == 'update') {
            if (!Validator::intVal()->validate($args['type_id'])) {
                $errors[] = 'type_id is not a numeric';
            } else {
                $obj = DoctypeModel::getById(['id' => $args['type_id']]);
            }

            if (empty($obj)) {
                $errors[] = 'Id ' . $args['type_id'] . ' does not exists';
            }
        }

        if (
            !Validator::notEmpty()->validate($args['description']) ||
        ...
        if (
            !Validator::notEmpty()->validate($args['doctypes_second_level_id']) ||
            !Validator::intVal()->validate($args['doctypes_second_level_id'])
        ) {
            $errors[] = 'Invalid doctypes_second_level_id value';
        }
        if (
            !Validator::notEmpty()->validate($args['process_delay']) &&
            (!Validator::intVal()->validate($args['process_delay']) || $args['process_delay'] < 0)
        ) {
            $errors[] = 'Invalid process_delay value';
        }
        if (
            !Validator::notEmpty()->validate($args['delay1']) &&
            (!Validator::intVal()->validate($args['delay1']) || $args['delay1'] < 0)
        ) {
            $errors[] = 'Invalid delay1 value';
        }
        if (
            !Validator::notEmpty()->validate($args['delay2']) &&
            (!Validator::intVal()->validate($args['delay2']) || $args['delay2'] < 0)
        ) {
            $errors[] = 'Invalid delay2 value';
        }
        if (
            Validator::notEmpty()->validate($args['duration_current_use'] ?? null) &&
            (!Validator::intVal()->validate($args['duration_current_use'] ?? null) ||
                ($args['duration_current_use'] ?? 0) < 0)
        ) {
