...
abstract class AttachmentModelAbstract
{
    ...
    public static function hasAttachmentsSignedByResId(array $args): bool
    {
        ...
        $attachments = DatabaseModel::select([
            'select' => [1],
            'table'  => ['res_attachments'],
            'where'  => ['res_id_master = ?', 'signatory_user_serial_id = ?', 'status <> ?'],
            'data'   => [$args['resId'], $args['userId'], 'DEL'],
        ]);
     ...
     * @throws Exception
     */
    public static function getWithRecursive(array $aArgs): array
    {
        ValidatorModel::notEmpty($aArgs, ['select', 'where', 'data', 'recursiveKey']);
        ValidatorModel::arrayType($aArgs, ['select', 'where', 'data']);
        ValidatorModel::stringType($aArgs, ['recursiveKey']);
        ...
        return DatabaseModel::selectRecursive([
            'select'       => $aArgs['select'],
            'table'        => 'res_attachments',
            'recursiveKey' => $aArgs['recursiveKey'],
            'where'        => $aArgs['where'],
            'data'         => $aArgs['data'],
            'order_by'     => empty($aArgs['orderBy']) ? [] : $aArgs['orderBy'],
