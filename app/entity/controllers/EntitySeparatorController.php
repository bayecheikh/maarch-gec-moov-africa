    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        foreach ($entitiesList as $entityId => $entityLabel) {
            ...
            if ($body['type'] == 'qrcode') {
                ...
                $pdf->Image('@' . $qrCodeResult, 0, 40, 200, 0, '', '', '', false, 300, 'C');
            } else {
                $barcode = new Barcode();
                $bobj = $barcode->getBarcodeObj(
                    'C128',                     // barcode type and additional comma-separated parameters
                    $prefix . $entityId,        // data string to encode
                    -4,                         // bar width (use absolute or negative value as a multiplication factor)
                    -100,
                    // bar height (use absolute or negative value as a multiplication factor)
                    'black',                    // foreground color
                    [-2, -2, -2, -2]       // padding (use absolute or negative values as multiplication factors)
                )->setBackgroundColor('white'); // background color

                $pdf->Image('@' . $bobj->getPngData(), 0, 40, 200, 0, '', '', '', false, 300, 'C');
            }
