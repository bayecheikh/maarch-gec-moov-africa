...
class VersionUpdateController
{
    ...
    /**
     * Get any tag folders that are superior to the current database version
    ...
    public static function executeTagSqlFile(string $sqlFilePath, string $docserverMigrationFolderPath): bool
    {
        ...
        if (file_exists($sqlFilePath)) {
            ...
            foreach ($explodedFile as $key => $line) {
                if (str_contains($line, '--DATABASE_BACKUP')) {
                    $lineNb = $key;
                }
            }
    ...
    public static function runScriptsByTag(array $folderFiles, string $tagVersion): array
    {
        ...
        foreach ($folderFiles as $fileName) {
            ...
            try {
                ...
                if (!$migration instanceof AutoUpdateInterface) {
                    LogsController::add([
                        'isTech'    => true,
                        'moduleId'  => 'Version Update',
                        'eventId'   => 'Run Scripts By Tag',
                        'level'     => 'CRITICAL',
                        'eventType' => "Class '$migrationClass' from '$filePath' does not implement AutoUpdateInterface"
                    ]);
                    $errors++;
                    continue;
                }
                ...
                LogsController::add([
                    ...
                    'eventId'   => 'Run Scripts By Tag',
                    'level'     => 'CRITICAL',
                    'eventType' => "Error loading migration file '$filePath': " . $th->getMessage()
                ]);
                $errors++;
                continue;
            }
