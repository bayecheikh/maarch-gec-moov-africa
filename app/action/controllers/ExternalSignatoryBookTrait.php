...
use MaarchCourrier\ExternalSignatureBook\Infrastructure\Goodflag\Factory\GoodflagSendDocumentToSignFactory;
        ...
        if (!empty($loadedXml)) {
                ...
                foreach ($attachments as $attachment) {
                    if ($attachment['status'] == 'SEND_MASS') {
                        $generated = AttachmentController::generateMailing(
                            ['id' => $attachment['res_id'], 'userId' => $GLOBALS['id']]
                        );
            ...
            if ($config['id'] == 'maarchParapheur') {
                ...
                $sentInfo = IxbusController::sendDatas([
                    'resIdMaster'  => $args['resId'],
                    'instanceId'   => $args['data']['ixbus']['instanceId'] ?? null,
            ...
            } elseif ($config['id'] == 'goodflag') {
                $sendToGoodflag = GoodflagSendDocumentToSignFactory::create();
                $sentInfo = $sendToGoodflag->execute(
                    $args['resId'],
