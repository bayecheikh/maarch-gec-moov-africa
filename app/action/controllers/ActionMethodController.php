...
use MaarchCourrier\DiffusionList\Infrastructure\Factory\AddUserEntityAsCopyActionFactory;
use MaarchCourrier\ExternalExport\Infrastructure\Maileva\MailevaShippingTrait;
use MaarchCourrier\SignatureBook\Infrastructure\Factory\ContinueCircuitActionFactory;
use MaarchCourrier\SignatureBook\Infrastructure\Factory\InterruptCircuitActionFactory;
use MaarchCourrier\SignatureBook\Infrastructure\Factory\RejectVisaBackToPreviousActionFactory;
use MaarchCourrier\SignatureBook\Infrastructure\Factory\Workflow\ResetWorkflowInSignatureBookActionFactory;
...
class ActionMethodController
{
    ...
    use MailevaShippingTrait;
    use ExternalSignatoryBookTrait;
    use ExportSEDATrait;

    public const COMPONENTS_ACTIONS_REQUIRE_ALL_RESOURCES = [
        'sendShippingAction' => 'sendMailevaShippingAction',
    ];
    public const COMPONENTS_ACTIONS = [
        ...
        'addUserEntityAsCopyAction'                 => 'addUserEntityAsCopyAction',
    ...
    public static function terminateAction(array $args): bool
    {
        ...
        if (empty($args['finishInScript'])) {
            ...
            if (!empty($status) && $status != '_NOSTATUS_') {
                $set['status'] = $status;
                $closingDate = in_array($status, $closedStatus) ? 'CURRENT_TIMESTAMP' : null;
                $set['closing_date'] = $closingDate;
            }
        } elseif (!empty($action['id_status']) && $action['id_status'] != '_NOSTATUS_') {
            $set['status'] = $action['id_status'];
            $closingDate = in_array($action, $closedStatus) ? 'CURRENT_TIMESTAMP' : null;
            $set['closing_date'] = $closingDate;
        }
    ...
    public static function redirect(array $args): bool|array
    {
        ...
        $alreadyInListinstances = [];
        foreach ($listInstances as $instance) {
            $alreadyInListinstances[] =
                $instance['item_id'] . "_" . $instance['item_type'] . "_" . $instance['item_mode'];
        }
        if (!empty($args['action']['parameters']['keepDestForRedirection'])) {
            $listInstancesDest = ListInstanceModel::get(
                [
                    'select' => ['*'],
                    'where'  => ['res_id = ?', 'difflist_type = ?', 'item_mode = ?'],
                    'data'   => [$args['resId'], 'entity_id', 'dest']
            ...
            if (!empty($listInstancesDest)) {
                $listInstancesDest[0]['item_mode'] = 'cc';
                if (
                    !in_array(
                        $listInstancesDest[0]['item_id'] . "_" . $listInstancesDest[0]['item_type'] . "_cc",
                        $alreadyInListinstances
                    )
                ) {
                    $listInstances[] = $listInstancesDest[0];
                }
            }
        }
     ...
     * @throws Exception
     */
    public static function addUserEntityAsCopyAction(array $args): bool|array
    {
        ValidatorModel::notEmpty($args, ['resId', 'data']);
        ValidatorModel::intVal($args, ['resId']);
        ValidatorModel::arrayType($args, ['data']);

        $addUserToCopy = AddUserEntityAsCopyActionFactory::create();
        $addUserToCopy->execute(
            $args['resId'],
            $args['data']
        );
    ...
    public static function rejectVisaBackToPrevious(array $args): bool|array
    {
        ...
        $env = new Environment();
        if ($env->isNewInternalParapheurEnabled()) {
            $rejectVisaBackToPreviousAction = RejectVisaBackToPreviousActionFactory::create();
            try {
                $basketOwner = (new User())
                    ->setId($args['userId']);
                $rejectVisaBackToPreviousAction->execute(
                    $args['resId'],
                    $basketOwner,
                    $args['basketId'],
                    $args['groupId'],
        ...
        // Here $listInstancesIdsToReset has at least one element
        // If not the error 'Workflow not yet started' will trigger before
        ListInstanceModel::update([
    ...
    public static function resetVisa(array $args): bool|array
    {
        ...
        try {
            $resetWorkflowInSignatureBook = (new ResetWorkflowInSignatureBookActionFactory())->create();
            $resetWorkflowInSignatureBook->reset($args['resId']);
        } catch (Throwable $th) {
            return ['errors' => [$th->getMessage()]];
        }
    ...
    public static function interruptVisa(array $args): bool|array
    {
        ...
        $env = new Environment();
        if ($env->isNewInternalParapheurEnabled()) {
            $interruptCircuitAction = InterruptCircuitActionFactory::create();
            try {
                $basketOwner = (new User())
                    ->setId($args['userId']);
                $interruptCircuitAction->execute(
                    $args['resId'],
                    $args['note'],
                    $basketOwner,
                    $args['basketId'],
