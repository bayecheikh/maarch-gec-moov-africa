...
class ShippingTemplateController
{
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        if ($shippingInfo['options']['sendMode'] == 'ere') {
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        if ($body['subscribed'] && !$alreadySubscribed) {
            ...
            $body['subscriptions'] = $subscriptions['subscriptions'];
            $iat = (int)$subscriptions['iat'];
            $body['token_min_iat'] = date_create_from_format('U', (string)($iat - 1));
    ...
    public function receiveNotification(Request $request, Response $response, array $args): Response
    {
        ...
        $initialStatus = ResModel::getById(['resId' => $resId, 'select' => ['status']]);
        $actionStatus = $initialStatus['status'];
    ...
    private static function checkData(array $args, string $mode): array
    {
        ...
        if ($args['options']['sendMode'] === 'ere') {
            if (!empty($args['fee'])) {
                foreach ($args['fee'] as $key => $fee) {
                    if ($key != 'ereSendingPrice' && $fee != 0) {
                        $errors[] = 'Only ereSendingPrice can be set for ERE mode';
                    }
                }
            }

            if (!empty($args['options']['shapingOptions'])) {
                $errors[] = 'Shaping options are not available for ERE mode';
            }

            if ($args['subscribed']) {
                $errors[] = 'Subscribed mode is not available for ERE mode';
            }

            if (empty($args['options']['senderId'])) {
                $errors[] = 'senderId is mandatory for ERE mode';
            }
        }
        return $errors;
    }
    ...
    private static function subscribeToNotifications(array $shippingTemplate): array
    {
        ...
        $maarchUrl = rtrim($configFile['config']['maarchUrl'], '/');
        if (empty($maarchUrl)) {
                ...
                if ($curlResponse['code'] != 201) {
                    return [
                        'errors' => 'Maileva POST/subscriptions returned HTTP ' .
                            $curlResponse['code'] .
                            '; ' .
                            json_encode($curlResponse['response'], JSON_THROW_ON_ERROR)
                    ];
                }
