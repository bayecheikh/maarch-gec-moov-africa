...
try {
    ...
    echo "[ExportSedaScript][Exception] {$e->getMessage()}";
}
...
class ExportSedaScript
{
    ...
    public static function send(array $args): bool
    {
        ...
        $config = [];
        $config['exportSeda'] = !empty($configuration['value']) ?
            json_decode($configuration['value'], true) :
            [];
        ...
        foreach ($args['data']['resources'] as $resId) {
            ...
            $elementSend = self::sendSedaPackage([
            ...
            if (!empty($elementSend['errors'])) {
                ResModel::update(
                    ['set' => ['status' => $args['data']['errorStatus']], 'where' => ['res_id = ?'], 'data' => [$resId]]
                );
                LogsController::add([
                    'isTech'    => true,
                    'moduleId'  => 'exportSeda',
                ...
                ResModel::update(
                    [
                        'set'   => ['status' => $args['data']['successStatus']],
                        'where' => ['res_id = ?'],
                        'data'  => [$resId]
                    ]
                );
