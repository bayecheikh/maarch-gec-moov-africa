...
trait PreProcessActionSEDATrait
{
     ...
     * @throws Exception
     */
    public function checkSendToRecordManagement(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if (empty($config['exportSeda']['senderOrgRegNumber'])) {
            return $response->withStatus(400)->withJson(
                ['errors' => 'No senderOrgRegNumber defined in parameters', 'lang' => 'noSenderOrgRegNumber']
            );
        }
        if (empty($config['exportSeda']['accessRuleCode'])) {
            return $response->withStatus(400)->withJson(
                ['errors' => 'No accessRuleCode defined in parameters', 'lang' => 'noAccessRuleCode']
            );
        }
        ...
        foreach ($destinationsData as $value) {
            if (!empty($value['producer_service'])) {
                if (!empty($producerService) && $producerService != $value['producer_service']) {
                    return $response->withStatus(400)->withJson(
                        ['errors' => 'All producer services are not the same', 'lang' => 'differentProducerServices']
                    );
                }
        ...
        if (empty($producerService)) {
            return $response->withStatus(400)->withJson(
                ['errors' => 'No accessRuleCode found in config.json', 'lang' => 'noProducerService']
            );
        }
        ...
        $recipientArchiveEntities = SedaController::getRecipientArchiveEntities(
            ['config' => $config, 'archivalAgreements' => $archivalAgreements['archivalAgreements']]
        );
        ...
        if ($massAction) {
            ...
            if (empty($actionParams['errorStatus']) || empty($actionParams['successStatus'])) {
                return $response->withStatus(403)->withJson(
                    [
                        'errors' => 'errorStatus or successStatus is not set for this action',
                        'lang'   => 'actionStatusNotSet'
                    ]
                );
            }
        }
        ...
        foreach ($body['resources'] as $resId) {
            if (empty($resources[$resId]['destination'])) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $resources[$resId]['alt_identifier'],
                    'res_id'         => $resId,
            ...
            if (!empty($resAcknowledgement[$resId])) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $resources[$resId]['alt_identifier'],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_alreadySent'
                ];
                continue;
            }
            ...
            if (
                empty($doctypesData[$typeId]['retention_rule']) ||
                empty($doctypesData[$typeId]['retention_final_disposition']) ||
                empty($doctypesData[$typeId]['duration_current_use'])
            ) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $resources[$resId]['alt_identifier'],
                    'res_id'         => $resId,
                    'reason'         => 'noRetentionInfo'
                ];
                continue;
            } else {
                if (
                    $resources[$resId]['binding'] === null &&
                    !in_array($doctypesData[$typeId]['action_current_use'], ['transfer', 'copy'])
                ) {
                    $resourcesInformations['errors'][] = [
                        'alt_identifier' => $resources[$resId]['alt_identifier'],
                        'res_id'         => $resId,
                        'reason'         => 'noTransferCopyRecordManagement'
                    ];
                    continue;
                } elseif (
                    $resources[$resId]['binding'] === true &&
                    !in_array($bindingDocument['param_value_string'], ['transfer', 'copy'])
                ) {
                    $resourcesInformations['errors'][] = [
                        'alt_identifier' => $resources[$resId]['alt_identifier'],
                        'res_id'         => $resId,
                        'reason'         => 'noTransferCopyBindingRecordManagement'
                    ];
                    continue;
                } elseif (
                    $resources[$resId]['binding'] === false &&
                    !in_array($nonBindingDocument['param_value_string'], ['transfer', 'copy'])
                ) {
                    $resourcesInformations['errors'][] = [
                        'alt_identifier' => $resources[$resId]['alt_identifier'],
                        'res_id'         => $resId,
                        'reason'         => 'noTransferCopyNoBindingRecordManagement'
                    ];
                    continue;
                }
                ...
                if (strtotime($date->format('Y-m-d')) >= time()) {
                    $resourcesInformations['errors'][] = [
                        'alt_identifier' => $resources[$resId]['alt_identifier'],
                        'res_id'         => $resId,
                        'reason'         => 'durationCurrentUseNotExceeded'
                    ];
                    continue;
                }
            }
            ...
            if (!empty($archivalData['errors'])) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $resources[$resId]['alt_identifier'],
                    'res_id'         => $resId,
                    'reason'         => $archivalData['errors']
                ];
                continue;
            } else {
                $archivalData['archivalData']['data']['metadata'] = [
                    'subject'        => $resources[$resId]['subject'],
                    'alt_identifier' => $resources[$resId]['alt_identifier']
                ];
            ...
            $unitIdentifier = MessageExchangeModel::getUnitIdentifierByResId(
                ['select' => ['message_id'], 'resId' => (string)$resId]
            );
            if (!empty($unitIdentifier[0]['message_id'])) {
                MessageExchangeModel::delete(
                    ['where' => ['message_id = ?'], 'data' => [$unitIdentifier[0]['message_id']]]
                );
            }
            $selectedResId[] = $resId;
        }
     ...
     * @throws Exception
     */
    public function checkAcknowledgementRecordManagement(Request $request, Response $response, array $args): Response
    {
        $body = $request->getParsedBody();
        ...
        $resourceAltIdentifier = ResModel::get(
            ['select' => ['alt_identifier', 'res_id'], 'where' => ['res_id in (?)'], 'data' => [$body['resources']]]
        );
        ...
        foreach ($body['resources'] as $resId) {
            if (!in_array($resId, $resIdAcknowledgement)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_noAcknowledgement'
                ];
                continue;
            }
            ...
            if (empty($acknowledgementXml)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_acknowledgementNotReadable'
                ];
                continue;
            }
            ...
            if (empty($messageExchange)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_noAcknowledgementReference'
                ];
                continue;
            }

            $unitIdentifier = MessageExchangeModel::getUnitIdentifierByResId(
                ['select' => ['message_id'], 'resId' => $resId]
            );
            if ($unitIdentifier[0]['message_id'] != $messageExchange['message_id']) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_wrongAcknowledgement'
                ];
                continue;
            }
     ...
     * @throws Exception
     */
    public function checkReplyRecordManagement(Request $request, Response $response, array $args): Response
    {
        $body = $request->getParsedBody();
        ...
        foreach ($body['resources'] as $resId) {
            if (!in_array($resId, $resIdReply)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_noReply'
                ];
                continue;
            }
            ...
            if (empty($replyXml)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_replyNotReadable'
                ];
                continue;
            }
            ...
            if (empty($messageExchange)) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_noReplyReference'
                ];
                continue;
            }
            ...
            if ($unitIdentifier[0]['message_id'] != $messageExchange['message_id']) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_wrongReply'
                ];
                continue;
            }

            if ($body['resetAction'] && str_contains((string)$replyXml->ReplyCode, '000')) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_alreadyArchived'
                ];
                continue;
            }
            if (!$body['resetAction'] && !str_contains((string)$replyXml->ReplyCode, '000')) {
                $resourcesInformations['errors'][] = [
                    'alt_identifier' => $altIdentifiers[$resId],
                    'res_id'         => $resId,
                    'reason'         => 'recordManagement_rejectedReply'
                ];
                continue;
            }
