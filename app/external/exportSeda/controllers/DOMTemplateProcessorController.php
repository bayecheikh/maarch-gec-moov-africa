...
class DOMTemplateProcessorController extends \DOMXPath
{
    ...
    public function merge($node = null, $source = null)
    {
        ...
        $expression = "descendant-or-self::processing-instruction('merge') | 
            descendant-or-self::text()[contains(., '[?merge')] | 
            descendant-or-self::*/@*[contains(., '[?merge')]";
        $mergeNodes = $this->query(
            $expression,
            $node
        );
    ...
    {
        $textNodes = $this->query(
            "descendant-or-self::text()[contains(., '[?merge')] | descendant-or-self::*/@*[contains(., '[?merge')]",
            $node
        );
