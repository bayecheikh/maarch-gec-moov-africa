...
class SendMessageController
{
     ...
     * @throws Exception
     */
    public static function send(stdClass $messageObject, string $messageId, string $type): bool|array
    {
        ...
        if ($channel == 'url') {
            ...
            return $adapter->send($messageObject, $messageId, $type);
        } elseif ($channel == 'email') {
            $adapter = new AdapterEmailController();
            return $adapter->send($messageObject, $messageId);
        } else {
            return false;
        }
    }
    ...
    public static function generateMessageFile(array $aArgs = [], bool $isForSeda = false): string
    {
     ...
     * @throws Exception
     */
    public static function generateSedaFile(array $aArgs = []): array
    {
        $tmpPath = CoreConfigModel::getTmpPath();
        ...
        foreach ($messageObject->dataObjectPackage->attachments as $attachment) {
            ...
            if ($attachment->type == "mainDocument") {
                ...
                if (!isset($attachment->retentionRule)) {
                    ...
                    $attachment->retentionFinalDisposition = $messageObject->dataObjectPackage
                ...
                $seda2Message
                    ...
                    ->DescriptiveMetadata
                    ->ArchiveUnit[0]
                    ->ArchiveUnit[] = self::getArchiveUnit(
                        $attachment->type,
                        $attachment,
                        null,
                        $attachment->id,
    ...
    private static function generateZip(stdClass $seda2Message, string $tmpPath): string
    {
        $zip = new ZipArchive();
        ...
        $zip->addFile(
            $tmpPath . $seda2Message->MessageIdentifier->value . ".xml",
            $seda2Message->MessageIdentifier->value . ".xml"
        );
        ...
        if (isset($seda2Message->DataObjectPackage) && $seda2Message->DataObjectPackage) {
            foreach ($seda2Message->DataObjectPackage->BinaryDataObject as $binaryDataObject) {
                $zip->addFile(
                    $tmpPath . $binaryDataObject->Attachment->filename,
                    $binaryDataObject->Attachment->filename
                );
            }
        }
    ...
    private static function initMessage(stdClass $messageObject): stdClass
    {
        $date = new DateTime();
    ...
    private static function getBinaryDataObject(string $filePath, mixed $id): stdClass
    {
    ...
    private static function getArchiveUnit(
        ...
        mixed $attachments = null,
        ?string $archiveUnitId = null,
        ?string $dataObjectReferenceId = null,
        ?array $relatedObjectReference = null
    ): stdClass {
    ...
    private static function getManagement(?stdClass $valueInData = null): stdClass
    {
        ...
        if ($valueInData->retentionRule) {
            ...
            if (
                isset($valueInData->retentionFinalDisposition) &&
                $valueInData->retentionFinalDisposition == "Conservation"
            ) {
     ...
     * @throws Exception
     */
    private static function getCustodialHistoryItem(stdClass $history): stdClass
    {
    ...
    private static function getContactData(stdClass $informations): stdClass
    {
