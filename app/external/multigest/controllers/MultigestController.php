...
class MultigestController
{
    ...
    public function createAccount(Request $request, Response $response): Response
    {
        ...
        foreach ($body['entities'] as $entity) {
            if (!Validator::notEmpty()->intVal()->validate($entity)) {
                return $response->withStatus(400)->withJson([
                    'errors' => 'Body entities contains non integer values'
                ]);
            }
        }
        ...
        EntityModel::update([
            'postSet' => [
                'external_id' => "jsonb_set(coalesce(external_id, '{}'::jsonb), '{multigest}', '{$account}')"
            ],
            'where'   => ['id in (?)'],
            'data'    => [$body['entities']]
        ]);
    ...
    public function updateAccount(Request $request, Response $response, array $args): Response
    {
        ...
        $account = [
            ...
            'password' => empty($body['password']) ?
                $externalId['multigest']['password'] :
                PasswordController::encrypt(['dataToEncrypt' => $body['password']]),
            'sasId'    => $body['sasId']
        ];
        $account = json_encode($account);
    ...
    public static function getResourceField(array $document, string $field, array $rawContacts): mixed
    {
        ...
        if (str_starts_with($field, 'senderAddress_')) {
            ...
            if (!empty($rawContacts[$contactIndex])) {
                return ContactController::getFormattedContactWithAddress(
                    ['contact' => $rawContacts[$contactIndex]]
                )['contact']['otherInfo'];
            } else {
                return '';
            }
     ...
     * @return int
     */
    private static function fieldNameToIndex(string $field): int
    {
