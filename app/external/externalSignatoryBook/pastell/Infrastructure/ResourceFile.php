...
class ResourceFile implements ResourceFileInterface
{
    ...
    public function getMainResourceFilePath(int $resId): string
    {
        ...
        if (
            ...
            return $letterboxPath['path_template'] . str_replace('#', '/', $adrMainInfo['path']) .
    ...
    public function getAttachmentFilePath(int $resId, string $fingerprint): string
    {
        $adrInfo = AdrModel::getConvertedDocumentById(
            ['resId' => $resId, 'collId' => 'attachments_coll', 'type' => 'PDF']
        );
        if (
        ...
        $annexeAttachmentPath = DocserverModel::getByDocserverId(
            ['docserverId' => $adrInfo['docserver_id'], 'select' => ['path_template', 'docserver_type_id']]
        );
        $filePath = $annexeAttachmentPath['path_template'] . str_replace(
            '#',
            DIRECTORY_SEPARATOR,
            $adrInfo['path']
        ) .
        ...
        $realFingerprint = StoreController::getFingerPrint(
