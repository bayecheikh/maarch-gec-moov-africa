...
class ParseIParapheurLog
{
    ...
    public function parseLogIparapheur(int $resId, string $idFolder): array
    {
        ...
        if ($iParapheurHistory->MessageRetour->codeRetour == $this->pastellStates->getErrorCode()) {
            return [
                'error' => 'Log KO in iParapheur : [' . $iParapheurHistory->MessageRetour->severite . '] ' .
                    $iParapheurHistory->MessageRetour->message
            ];
        }
        foreach ($iParapheurHistory->LogDossier->LogDossier as $historyLog) {
            ...
            if ($status == $this->pastellStates->getSignState()) {
                ...
                $return['signatory'] = isset($historyLog->nom) ? (string)$historyLog->nom : '';
                $annotation = isset($historyLog->annotation) ? (string)$historyLog->annotation : '';
                $return['notes'] = [['content' => $return['signatory'] . ' : ' . $annotation]];
                break;
                ...
                $status == $this->pastellStates->getRefusedSign() ||
                $status == $this->pastellStates->getRefusedVisa()
            ) {
                $nom = isset($historyLog->nom) ? (string)$historyLog->nom : '';
                $annotation = isset($historyLog->annotation) ? (string)$historyLog->annotation : '';
                $return = $this->handleRefused($nom, $annotation);
                $return['signatory'] = $nom;
                $return['notes'] = [['content' => $return['content']]];
                break;
            }
        }
        if (empty($return)) {
