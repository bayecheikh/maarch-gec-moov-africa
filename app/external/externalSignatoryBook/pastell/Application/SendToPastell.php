...
class SendToPastell
{
    ...
    public function sendResource(int $resId, string $sousType): array
    {
        ...
        if (
            $mainDocumentIntegration['inSignatureBook'] &&
            empty(json_decode($mainResource['external_id'], true))
        ) {
    ...
    public function sendFolderToPastell(
        ...
        if (in_array('send-iparapheur', $info['actionPossibles'])) {
            ...
            if (!$sendIparapheur) {
                $info = "L'action « send-iparapheur »  n'est pas permise : ";
                $info .= "Le dernier état du document (send-iparapheur) ne permet pas de déclencher cette action";
                return ['error' => $info];
            }
        }
