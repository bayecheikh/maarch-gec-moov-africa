...
class RetrieveFromPastell
{
    ...
    public function retrieve(array $idsToRetrieve, string $documentType): array
    {
        ...
        foreach ($idsToRetrieve as $key => $value) {
            ...
            $value['external_id'] = (string)($externalId['signatureBookId'] ?? '');
            ...
            if (!empty($info['error'])) {
                ...
                $this->historyRepository->addLogInHistory(
                    $value['res_id_master'] ?? $value['res_id'],
                    'Error when getting folder detail : ' . $infosError
                );

                if (
                    $info['code'] == 404 &&
                    $info['error'] ==
                    "Le document {$value['external_id']} n'appartient pas à l'entité {$this->config->getEntity()}"
                ) {
                    try {
                        ...
                        $errors[$key] = "[SCRIPT] Failed to remove document link: " .
                            "MaarchCourrier docId {$value['res_id']}, document type $type," .
                            " parapheur docId {$value['external_id']}";
                ...
                if (in_array('verif-iparapheur', $info['actionPossibles'] ?? [])) {
                    ...
                    if ($verif !== true) {
                        ...
                        $this->historyRepository->addLogInHistory(
                            $value['res_id_master'] ?? $value['res_id'],
                            'Action "verif-iparapheur" failed'
                        );
                        unset($idsToRetrieve[$key]);
                        continue;
                    }
                }
                // Need res_id_master for parseLogIparapheur
                // and res_id for updateDocument (for attachments and main document)
                $resId = $value['res_id_master'] ?? $value['res_id'];
                $result = $this->parseIParapheurLog->parseLogIparapheur($resId, $value['external_id']);
                $this->resourceData->updateDocumentExternalStateSignatoryUser(
                    $value['res_id'],
                    $type,
                    $result['signatory'] ?? ''
                );
                ...
                if (!empty($result['error'])) {
                    ...
                    $resultError = implode('-', (array)$result['error']);
                ...
                if (
                    ...
                    if (!empty($deleteFolderResult['error'])) {
                        ...
                        $deleteError = (is_array($deleteFolderResult['error'])) ? implode(
                            '-',
