...
class MaarchParapheurController
{
            ...
            if (empty($attachments) && empty($integratedResource)) {
                ...
                foreach ($attachments as $value) {
                    ...
                    if (!empty($response['response']['errors']) || !empty($response['errors'])) {
                        return [
                            'error' => 'Error during processing in MaarchParapheur : ' .
                ...
                if (!empty($integratedResource)) {
                    ...
                    $attachmentToFreeze['letterbox_coll'][$integratedResource[0]['res_id']] =
    ...
    public static function retrieveSignedMails(array $aArgs): array
    {
        ...
        foreach ($aArgs['idsToRetrieve'][$version] as $resId => $value) {
            ...
            if (!empty($documentWorkflow['errors'])) {
                LogsController::add([
                    'isTech'    => true,
                    ...
                    'eventType' => 'script',
                    'eventId'   => "[MaarchParapheur api] {$documentWorkflow['errors']}"
                ]);
                ...
                HistoryController::add([
                    ...
                    'eventId'   => '1',
                    'userId'    => $userId,
                    'info'      => "[MaarchParapheur api] {$documentWorkflow['errors']}"
                ]);
     ...
     * @return array
     */
    public static function getState(array $aArgs): array
    {
    ...
    public static function userStatusInMaarchParapheur(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if ($loadedXml->signatoryBookEnabled == 'maarchParapheur') {
            ...
            if (empty($url) || empty($userId) || empty($password)) {
                return $response->withStatus(400)->withJson([
                    'errors' => 'Could not get remote signatory book configuration.' .
        ...
        if ($loadedXml->signatoryBookEnabled == 'maarchParapheur') {
            ...
            if (!empty($externalId['maarchParapheur'])) {
                $userSignatures = UserSignatureModel::get([
                    'select' => ['signature_path', 'signature_file_name', 'id', 'docserver_id'],
                ...
                foreach ($userSignatures as $value) {
                    $docserver = DocserverModel::getByDocserverId(
                        ['docserverId' => $value['docserver_id'], 'select' => ['path_template']]
                    );
     ...
     * @throws Exception
     */
    public static function userExists(array $args): bool|array
    {
