...
use MaarchCourrier\ExternalSignatureBook\Application\Fast\FastHistoryStepMessageBuilder;
...
class FastParapheurController
{
    ...
    public static function retrieveSignedMails(array $args): array
    {
        ...
        foreach ($args['idsToRetrieve'][$version] as $resId => $value) {
            ...
            $value['external_id'] = $externalId['signatureBookId'];
            $eventIdInfo = empty($value['res_id_master']) ? "main document resId: $resId" : "attachment resId: $resId";

            $isMainResource = true;
            $docIdentifier = $value['alt_identifier'] ?? $resId . ' (res_letterbox)';
            if ($version != 'resLetterbox') {
                $isMainResource = false;
                $docIdentifier = $value['identifier'] ?? $resId . ' (res_attachments)';
            }
            ...
            if (
                ...
                $lastFastWorkflowAction = FastParapheurController::getLastFastWorkflowAction(
                    ...
                    $knownWorkflow,
                    $args['config']['data'],
                    $args['idsToRetrieve'][$version][$resId],
                    $isMainResource,
                    $docIdentifier
                );
            ...
            $historyMessageBuilder = new FastHistoryStepMessageBuilder(new HistoryRepository());

            // Loop on all steps of the documents (prepared, send to signature, signed etc...)
            foreach ($historyResponse['response'] as $keyResponse => $valueResponse) {
                ...
                $buildHistoryMessage = $historyMessageBuilder->buildMessage(
                    $isMainResource,
                    $docIdentifier,
                    $valueResponse['stateName'],
                    new DateTimeImmutable($valueResponse['date'] ?? 'now'),
                    $signatoryInfo['name'] ?? $valueResponse['userFullname'],
                );

                if (
                    !isset($args['idsToRetrieve'][$version][$resId]['history']) ||
                    !in_array($keyResponse, array_keys($args['idsToRetrieve'][$version][$resId]['history'])) &&
                    $buildHistoryMessage !== null
                ) {
                    $args['idsToRetrieve'][$version][$resId]['history'][$keyResponse] = $buildHistoryMessage;
                }
    ...
    public static function getSignatoryUserInfo(array $args = []): array
    {
        ...
        if (
            ...
            $user = DatabaseModel::select([
                'select'    => ['users.id', "CONCAT(firstname, ' ', lastname) as name"],
    ...
    public static function prepareDocumentsToSign(
        ...
        if (
            ...
            $externalState = json_decode($mainResource['external_state'] ?? '{}', true);
            $externalState['signatureBookWorkflow']['workflow'] = $workflowSteps;
            ResModel::update([
                'where'   => ['res_id = ?'],
    ...
    public static function onDemandUploadFilesToFast(array $args): array
    {
        ...
        $multipartBody = [
            ...
            'circuit' => json_encode($args['circuit'], JSON_PRETTY_PRINT)
        ];

        if (!empty($args['document']['appendices'])) {
            $multipartBody['annexes']['subvalues'] = $args['document']['appendices'];
        }
    ...
    {
        try {
            $fastConfig = (new RetrieveConfig(new ExternalSignatureBookConfigService()))->get();
        } catch (Exception $e) {
            return ['code' => $e->getCode(), 'errors' => $e->getMessage()];
        }
        return $fastConfig->jsonSerialize();
    }
     ...
     * @throws Exception
     */
    public static function getLastFastWorkflowAction(
        array $documentHistory,
        array $knownWorkflow,
        array $config,
        array &$returnedWorkflows,
        bool $isMainResource,
        ...
        foreach ($documentHistory as $stepKey => $historyStep) {
            ...
            $buildHistoryMessage = $historyMessageBuilder->buildMessage(
                $isMainResource,
                $docIdentifier,
                $historyStep['stateName'],
                new DateTimeImmutable($historyStep['date'] ?? 'now'),
                $historyStep['userFullname']
            );

            if ($buildHistoryMessage !== null) {
                $returnedWorkflows['history'][$stepKey] = $buildHistoryMessage;
            }
            ...
            if (in_array($historyStep['stateName'], [$config['refusedState'], $config['refusedVisaState']])) {
                ...
                break;
            } elseif (in_array($historyStep['stateName'], [$config['validatedState'], $config['validatedVisaState']])) {
