...
class AnnuaryController
{
     ...
     * @throws Exception
     */
    public static function updateEntityToOrganization(Request $request, Response $response, array $args): Response
    {
        ...
        if (empty($siret['param_value_string'])) {
            return $response->withStatus(400)->withJson([
                'errors' => 'Parameter siret does not exist',
                'lang'   => 'invalidToken'
            ]);
        }
        ...
        foreach ($annuaries as $annuary) {
            ...
            if (!$authenticated) {
                return $response->withStatus(400)->withJson([
                    'errors' => 'Ldap authentication failed : ' . ldap_error($ldap)
                ]);
            }
            ...
            if ((int)$entries['count'] == 0) {
                ...
                if (!$added) {
                    return $response->withStatus(400)->withJson([
                        'errors' => 'Ldap add failed : ' . ldap_error($ldap)
                    ]);
                }
            }
            ...
            $search = @ldap_search(
                $ldap,
                "ou={$organization},{$annuary['baseDN']}",
                "(initials={$entity['entity_id']})",
                ['dn']
            );
            if ($search === false) {
                return $response->withStatus(400)->withJson([
                    'errors' => 'Ldap search failed : ' . ldap_error($ldap)
                ]);
            }
            $entries = ldap_get_entries($ldap, $search);
            ...
            if ((int)$entries['count'] > 0) {
                $renamed = @ldap_rename(
                    $ldap,
                    $entries[0]['dn'],
                    "cn={$entity['entity_label']}",
                    "ou={$organization},{$annuary['baseDN']}",
                    true
                );
                if (!$renamed) {
                    return $response->withStatus(400)->withJson([
                        'errors' => 'Ldap rename failed : ' . ldap_error($ldap)
                    ]);
                }

                $replaced = @ldap_mod_replace(
                    $ldap,
                    "cn={$entity['entity_label']},ou={$organization},{$annuary['baseDN']}",
                    ['sn' => $entity['entity_label']]
                );
                if (!$replaced) {
                    return $response->withStatus(400)->withJson([
                        'errors' => 'Ldap replace failed : ' . ldap_error($ldap)
                    ]);
                }
            } else {
                $info = [];
    ...
    public static function addContact(array $args): array
    {
        ...
        foreach ($annuaries as $annuary) {
            ...
            if ((int)$entries['count'] > 0) {
                ...
                $search = @ldap_search(
                    $ldap,
                    "ou={$entries[0]['ou'][0]},{$annuary['baseDN']}",
                    "(initials={$entityId})",
                    ['ou', 'entryUUID']
                );
        ...
        foreach ($annuaries as $annuary) {
            ...
            if ((int)$entries['count'] > 0) {
                ...
                $search = @ldap_search(
                    ...
                    "{$annuary['baseDN']}",
                    "({$parentOu})",
                    ['dn', 'destinationIndicator', 'postOfficeBox', 'labeledURI']
                );
                $entries = ldap_get_entries($ldap, $search);

                return [
                    'mail'       => $entries[0]['postofficebox'][0],
                    'url'        => $entries[0]['labeleduri'][0],
                    'businessId' => $entries[0]['destinationindicator'][0] . '/' . $departmentDestinationIndicator
                ];
            }
