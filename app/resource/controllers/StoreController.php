...
use MaarchCourrier\DocumentStorage\Infrastructure\Service\PdfFileNormalizationService;
use MaarchCourrier\SignatureBook\Infrastructure\Service\CheckDocumentSignatureFieldService;
...
class StoreController
{
    ...
    public static function storeResource(array $args): array
    {
        try {
            ...
            if (!empty($args['encodedFile'])) {
                ...
                //Check if electronic certificat is applied on main resource only for PDF
                if ($args['format'] == 'pdf') {
                    $checkSignatureService = new CheckDocumentSignatureFieldService();
                    $environnement = new Environment();
                    $hasElectronicCertificate = $checkSignatureService->checkDocumentSignatureFields(
                        $args['encodedFile']
                    );

                    $data['external_state']['hasDigitalSignature'] = $hasElectronicCertificate;

                    if (isset($data['external_state']) && is_array($data['external_state'])) {
                        $data['external_state'] = json_encode($data['external_state']);
                    }

                    if (!$hasElectronicCertificate && $environnement->isPdfNormalizationEnabled()) {
                        $pdfFileNormalizationService = new PdfFileNormalizationService();
                        $fileContent = $pdfFileNormalizationService->normalize($fileContent);
                    }
                }
     ...
     * @return int|array
     */
    public static function storeAttachment(array $args, ?array $queryParams = null): array|int
    {
        try {
            if (empty($args['id'])) {
                ...
                $data = $args;
                $data['resId'] = $resId;
                $data = StoreController::prepareAttachmentStorage($data, $queryParams);
            } else {
                $resId = $args['id'];
            ...
            if (!empty($args['encodedFile'])) {
                ...
                //Check if electronic certificat is applied on attachment for PDF
                if ($args['format'] == 'pdf') {
                    $checkSignatureService = new CheckDocumentSignatureFieldService();
                    $environnement = new Environment();
                    $hasElectronicCertificate = $checkSignatureService->checkDocumentSignatureFields(
    ...
    public static function prepareUpdateResourceStorage(array $args): array
    {
        ...
        if (!empty($args['templateId'])) {
            $preparedData['template_id'] = $args['templateId'];
        }

        if (isset($args['isAnnotated'])) {
            $preparedData['is_annotated'] = !$args['isAnnotated'] ? 'false' : 'true';
        }
     ...
     * @throws Exception
     */
    public static function prepareAttachmentStorage(array $args, ?array $queryParams = null): array
    {
        ...
        $resContactsOrUsers = ResourceContactModel::get([
            'select'  => ['item_id', 'type'],
            'where'   => ['res_id = ?', 'type in (?)', 'mode in (?)'],
            'data'    => [$args['resIdMaster'], ['contact', 'user'], ['sender', 'recipient']],
            'orderBy' => ['id ASC']
        ]);
        if (
            !empty($resContactsOrUsers) &&
            empty($args['recipientId'] ?? null) &&
            empty($args['recipientType'] ?? null)
        ) {
            if (count($resContactsOrUsers) == 1) {
                $args['recipientId'] = $resContactsOrUsers[0]['item_id'];
                $args['recipientType'] = $resContactsOrUsers[0]['type'];
                $args['status'] = 'A_TRA';
            } else {
                $contacts = array_filter($resContactsOrUsers, function ($contact) {
                    return $contact['type'] == 'contact';
                });
                if (count($contacts) >= 2) {
                    $args['status'] = 'SEND_MASS';
                }
            }
        }
        ...
        $signaturePositions = $args['signature_positions'] ?? [];
        if (!empty($args['originId'])) {
            $relations = AttachmentModel::get(
                [
                    'select'  => ['relation', 'in_signature_book', 'template_id', 'signature_positions'],
            ...
            //Récupération templateId version précédente si argument withTemplate = true
            if (!empty($queryParams) && $queryParams['withTemplate'] === 'true') {
                $templateId = $relations[0]['template_id'];
            } elseif (!empty($queryParams) && $queryParams['withTemplate'] === 'false') {
                $templateId = null;
            }

            if (empty($signaturePositions)) {
                $signaturePositions = json_decode($relations[0]['signature_positions'], true);
            }
        }
        ...
        return [
            ...
            'signature_positions'      => json_encode($signaturePositions),
            'is_annotated'             => ($args['is_annotated'] ?? false) ? 'true' : 'false'
        ];
    }
    ...
    public static function prepareUpdateAttachmentStorage(array $args): array
    {
        ...
        if (!empty($args['type']) && $attachmentsTypes[$args['type']] && empty($attachment['identifier'])) {
            $resource = ResModel::getById(
        ...
        if (!empty($args['onlyDocument'])) {
            ...
            if (!empty($args['inSignatureBook'])) {
                $preparedData['in_signature_book'] = 'true';
            }
        }
