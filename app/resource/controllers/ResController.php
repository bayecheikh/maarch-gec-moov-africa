...
class ResController extends ResourceControlController
{
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (!empty($body['format']) && strtolower($body['format']) === 'pdf') {
            $launchLadProcess = new LaunchLadProcess(new MercureService());
            $ladResult = $launchLadProcess->execute($body['encodedFile'], $body['format'], $body['modelId']);

            foreach ($ladResult as $ladKey => $ladValue) {
                if ($ladKey === 'contactIdx' && empty($body['senders']) && !empty($ladValue)) {
                    $body['senders'][] = ['type' => 'contact', 'id' => $ladValue];
                } elseif (str_contains($ladKey, 'indexingCustomField_')) {
                    $customField = explode('_', $ladKey);
                    if (empty($body['customFields'][$customField[1]]) && !empty($ladValue)) {
                        $customFieldConfig = CustomFieldModel::getById(["id" => $customField[1]]);
                        if (
                            $customFieldConfig['type'] === 'date' ||
                            $customFieldConfig['type'] === 'string' ||
                            $customFieldConfig['type'] === 'integer'
                        ) {
                            $body['customFields'][$customField[1]] = $ladValue;
                        } elseif (
                            $customFieldConfig['type'] === 'radio' ||
                            $customFieldConfig['type'] === 'select'
                        ) {
                            if (in_array($ladValue, $customFieldConfig['values'])) {
                                $body['customFields'][$customField[1]] = $ladValue;
                            }
                        }
                    }
                } elseif (empty($body[$ladKey]) && !empty($ladValue)) {
                    $body[$ladKey] = $ladValue;
                }
            }
        }
        ...
        if (!empty($storedResource['errors'])) {
            return $response->withStatus(500)->withJson(
                ['errors' => '[ResController create] ' . $storedResource['errors']]
            );
        }
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        foreach ($formattedData as $key => $data) {
            if (!in_array($key, $modelFields) && $key !== 'template') {
        ...
        if (!empty($formattedData['template'])) {
            $templateRepository = new TemplateRepository();
            $formattedData['template'] = $templateRepository->getById($formattedData['template']);
        }
        ...
        $formattedData['isAnnotated'] = $document['is_annotated'];
        if ($formattedData['isAnnotated']) {
            $templateRepository = new TemplateRepository();
            ...
            if (!empty($notAnnotatedResourceVersion)) {
                ...
                try {
                    $originalMainFile = $retrieveResourceFactory->getResourceFile(
                        $args['resId'],
                        false,
                        $specificVersion
                    );
                    $formattedData['lastUnannotatedVersionFormat'] = $originalMainFile->getPathInfo()['extension'];
                } catch (Throwable $th) {
                ...
                return $response->withStatus(400)->withJson([
                    'errors' => 'Not annotated version of document is not found'
                ]);
            }
        }
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        if (!empty($resource['filename']) && !empty($body['encodedFile'])) {
            AdrModel::createDocumentAdr([
                ...
                'is_annotated' => $resource['is_annotated'] == 'true'
            ]);
    ...
    private static function createAdjacentData(array $args): void
    {
        ...
        if (!empty($body['diffusionList'])) {
            foreach ($body['diffusionList'] as $diffusion) {
                if ($diffusion['mode'] == 'dest' && $diffusion['type'] == 'user') {
                    ResModel::update(
                        [
            ...
            if (!empty($listTemplate)) {
                ...
                if (!empty($listTemplateItems)) {
                    foreach ($listTemplateItems as $diffusion) {
                        if ($diffusion['item_mode'] == 'dest' && $diffusion['item_type'] == 'user') {
                            ResModel::update(
                                [
                                    'set'   => ['dest_user' => $diffusion['item_id']],
                                    'where' => ['res_id = ?'],
                                    'data'  => [$args['resId']]
                                ]
                            );
    ...
    public function getResourceFileInformation(Request $request, Response $response, array $args): Response
    {
        ...
        if (!empty($resource['docserver_id'])) {
            $resource['docserverPathFile'] = "{" . $resource['docserver_id'] . "}" .
                DIRECTORY_SEPARATOR . $resource['path'];
