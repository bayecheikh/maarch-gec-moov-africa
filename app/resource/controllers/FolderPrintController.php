...
class FolderPrintController
{
    ...
    public function generateFile(Request $request, Response $response): Response
    {
        ...
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'print_folder_doc', 'userId' => $GLOBALS['id']])) {
        ...
        foreach ($body['resources'] as $resource) {
            ...
            if (!empty($documentPaths)) {
                if (empty($resource['altIdentifier']) && empty($resource['subject'])) {
                    $document = ResModel::getById([
                        'select' => ['alt_identifier', 'subject'],
                        'resId'  => $resource['resId']
                    ]);
                    $resource['altIdentifier'] = $document['alt_identifier'] ?? 'MAARCH';
                    $resource['subject'] = $document['subject'] ?? $resource['resId'];
                }
                ...
                try {
                    $documentPaths = array_filter($documentPaths, fn($path) => is_string($path));

                    $command = "pdfunite '" . implode("' '", $documentPaths) . "' '" . $filePathOnTmp . "'";
                    exec($command . ' 2>&1', $output, $return);

                    if ($return != 0) {
                        if (str_contains(implode("\n", $output), "Could not merge encrypted files")) {
                            $protectedFiles = [];
                            $resourceData = new ResourceData();

                            foreach ($documentPaths as $path) {
                                if (FolderPrintController::isProtected($path)) {
                                    $altIdentifier = $resourceData->getAltIdentifierByFilename(basename($path));
                                    if (!empty($altIdentifier)) {
                                        $protectedFiles[] = $altIdentifier;
                                    }
                                }
                            }
                            $errorMessage = _ERROR_ENCRYPTED_FILE . implode(", ", $protectedFiles);
                        } else {
                            $errorMessage = 'Error pdfunite : ' . implode("\n", $output);
                        }
                        return $response->withStatus(500)->withJson(['errors' => $errorMessage]);
                    }
                } catch (Exception $e) {
                    return $response->withStatus(500)->withJson([
                        'errors' => '[FolderPrintController] Unexpected error : ' . $e->getMessage()
                    ]);
                }
    ...
    public static function isProtected(string $filePath): bool
    {
        ...
        // Lire les premiers octets du fichier pour vérifier la présence d'encryption
        ...
        // Vérifie la présence de marqueurs d'encryption dans le PDF
        return (
            str_contains($content, '/Encrypt') ||
            str_contains($content, '/Encryption') ||
            str_contains($content, '/EFF')
        );
    }
}
