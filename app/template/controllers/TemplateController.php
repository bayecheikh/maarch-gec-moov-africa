...
class TemplateController
{
    ...
    public function getDetailledById(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if (in_array($template['template_target'], ['indexingFile', 'attachments'])) {
            $template['signaturePositions'] = json_decode($template['signature_positions'] ?? '[]', true);
            unset($template['signature_positions']);
        }
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (in_array($body['target'], ['indexingFile', 'attachments'])) {
            $signaturePositions = json_encode($body['signaturePositions'] ?? []);
        }
        ...
        if (
            ...
            $template['docserver_id'] = $storeResult['docserver_id'];
        }
    ...
    public function update(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if (!empty($body['datasource'])) {
            $templateSet['template_datasource'] = $body['datasource'];
        }
        if (!empty($body['options'])) {
            if (
                !empty($body['options']['acknowledgementReceiptFrom']) && !in_array(
            ...
            $templateSet['options'] = json_encode($options);
        }
        if (
            $body['type'] == 'TXT' || $body['type'] == 'HTML' ||
            ($body['type'] == 'OFFICE_HTML' && !empty($body['file']['electronic']['content']))
        ) {
            $templateSet['template_content'] = $body['type'] ==
            'OFFICE_HTML' ? $body['file']['electronic']['content'] : $body['file']['content'];
        }
        if (
            ...
            $templateSet['template_path'] = $storeResult['destination_dir'];
            $templateSet['template_file_name'] = $storeResult['file_destination_name'];
            $templateSet['docserver_id'] = $storeResult['docserver_id'];
        }
        ...
        HistoryController::add([
            ...
            'recordId'  => $aArgs['id'],
            'eventType' => 'UP',
            'info'      => _TEMPLATE_UPDATED . " : {$templateSet['template_label']}",
            'moduleId'  => 'template',
    ...
    public function delete(Request $request, Response $response, array $aArgs): Response
    {
        ...
        ResModel::update(['set' => ['template_id' => null], 'where' => ['template_id = ?'], 'data' => [$aArgs['id']]]);
        AttachmentModel::update(
            ['set' => ['template_id' => null], 'where' => ['template_id = ?'], 'data' => [$aArgs['id']]]
        );
    ...
    public function duplicate(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if ($template['template_type'] == 'OFFICE') {
            $docserver = DocserverModel::getCurrentDocserver(
                ['typeId' => 'TEMPLATES', 'collId' => 'templates', 'select' => ['path_template', 'docserver_id']]
            );
            ...
            $template['docserver_id'] = $docserver['docserver_id'];
        }
    ...
    private static function controlCreateTemplate(array $aArgs): bool
    {
        ...
        if ($data['type'] == 'OFFICE_HTML') {
            $check = $check && (Validator::notEmpty()->validate($data['file']['paper']['content'] ?? null) ||
                    Validator::notEmpty()->validate($data['file']['electronic']['content'] ?? null));
