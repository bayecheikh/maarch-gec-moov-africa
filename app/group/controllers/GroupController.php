...
use MaarchCourrier\SignatureBook\Infrastructure\Factory\ReassignUsersToGroupInSignatoryBookFactory;
use MaarchCourrier\User\Domain\User;
use MaarchCourrier\SignatureBook\Infrastructure\Factory\Group\GetUsersFromGroupFactory;
...
class GroupController
{
    ...
    public function delete(Request $request, Response $response, array $aArgs): Response
    {
        ...
        if ($env->isNewInternalParapheurEnabled()) {
            $groupToDelete = (new Group())
                ->setExternalId($externalId);
            $deleteGroup = (new DeleteGroupInSignatoryBookFactory())->create();

            $config = CoreConfigModel::getJsonLoaded(['path' => 'config/config.json']);
            if (!empty($config['signatureBook']['userIdParapheurWS']) && !empty($externalId['internalParapheur'])) {
                $getUsersFromGroupFactory = new GetUsersFromGroupFactory();
                $getUsersFromGroup = $getUsersFromGroupFactory->create();
                $users = $getUsersFromGroup->execute($groupToDelete);
                if (!empty($users)) {
                    foreach ($users as $user) {
                        if ($user['email'] === $config['signatureBook']['userIdParapheurWS']) {
                            return $response->withStatus(400)->withJson([
                                'errors' => _ERROR_DELETION_GROUP_MP_WEBSERVICE
                            ]);
                        }
                    }
                }
    ...
    public function reassignUsers(Request $request, Response $response, array $aArgs): Response
    {
        ...
        $newGroup = GroupModel::getById(['id' => $aArgs['newGroupId'], 'select' => ['group_id', 'external_id']]);
        if (empty($newGroup)) {
            return $response->withStatus(400)->withJson(['errors' => 'Group not found']);
        }
        $oldGroupUsers = GroupModel::getUsersById(
            [
                'id' => $aArgs['id'], 'select' => ['users.id', 'users.external_id', 'users.user_id']
            ]
        );
        ...
        if ($env->isNewInternalParapheurEnabled()) {
            $usersToReassign = [];
            foreach ($users as $user) {
                $usersExternalId = json_decode($user['external_id'], true);
                $user = (new User())
                    ->setId($user['id'])
                    ->setLogin($user['login'])
                    ->setExternalId($usersExternalId);
                $usersToReassign[] = $user;
            }

            $groupExternalId = json_decode($newGroup['external_id'], true);
            $group = (new Group())->setExternalId($groupExternalId);

            $reassignUsers = (new ReassignUsersToGroupInSignatoryBookFactory())
                ->create();
            $reassignUsers->reassignUsers($group, $usersToReassign);
        }
