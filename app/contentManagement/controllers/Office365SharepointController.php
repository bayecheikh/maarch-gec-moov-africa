...
class Office365SharepointController
{
    ...
    public function sendDocument(Request $request, Response $response, array $args): Response
    {
        ...
        if (empty($body['encodedContent'])) {
            $document = CollaboraOnlineController::getDocument([
                ...
                'format'                => $body['format'],
                'path'                  => $body['path'] ?? null,
                'getUnannotatedVersion' => $body['unannotatedVersion'] ?? false,
            ]);
    ...
    private static function deleteFile(array $args): void
    {
        ...
        if ($result['code'] != 204) {
            if (!empty($result['response']['errors'])) {
                $info = json_encode($result['response']['errors']);
            } elseif (!empty($result['message'])) {
                $info = json_encode($result['errors']);
            } else {
                $info = 'No errors from curl -> response -> errors and in curl -> errors';
            }
            ...
            LogsController::add([
                ...
                'tableName' => 'letterbox_coll',
                'recordId'  => $args['resId'],
                'eventType' => "Failed to delete document {$args['documentId']} : $info",
                'eventId'   => "resId : {$args['resId']}"
            ]);
        }
    }
