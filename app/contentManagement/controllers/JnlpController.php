...
class JnlpController
{
    ...
    public function generateJnlp(Request $request, Response $response): Response
    {
        ...
        $unannotatedVersion = $body['unannotatedVersion'] ?? false;
        ...
        $jnlpFile = fopen($tmpPath . $jnlpFileName . '.lck', 'w+');
        fwrite($jnlpFile, $unannotatedVersion);
        ...
        if ($queryParams['action'] == 'editObject') {
            if ($queryParams['objectType'] == 'templateCreation') {
                ...
                $template = TemplateModel::getById([
                    ...
                    'select' => ['template_path', 'template_file_name', 'docserver_id']
                ]);
                if (empty($template)) {
                ...
                $tmpPath = CoreConfigModel::getTmpPath();
                $lockFileName = $tmpPath . "{$GLOBALS['id']}_maarchCM_{$args['jnlpUniqueId']}.lck";

                $lockFileContent = file_get_contents($lockFileName);

                $getUnannotatedVersion = filter_var(
                    $lockFileContent,
                    FILTER_VALIDATE_BOOLEAN
                );

                if ($getUnannotatedVersion) {
                    ...
                    if (!empty($notAnnotatedResourceVersion)) {
                        ...
                        $xmlResponse = JnlpController::generateResponse(
                            ['type' => 'ERROR', 'data' => ['ERROR' => "Not annotated version of document is not found"]]
                        );
                        $response->write($xmlResponse);
                        return $response->withHeader('Content-Type', 'application/xml');
                    }

                    $document = $mainResourceRepository->getDocumentByResIdAndVersion(
                        $queryParams['objectId'],
                        $specificVersion
                    );

                    $resource['docserver_id'] = $document->getDocserverId();
                    $resource['path'] = $document->getPath();
                    $resource['filename'] = $document->getFileName();
                    $resource['fingerprint'] = $document->getFingerprint();
                }
                ...
                if ($getUnannotatedVersion) {
                    ...
                    if (!empty($notAnnotatedAttachment)) {
                        $attachment['docserver_id'] = $notAnnotatedAttachment->getDocument()->getDocserverId();
                        $attachment['path'] = $notAnnotatedAttachment->getDocument()->getPath();
                        $attachment['filename'] = $notAnnotatedAttachment->getFileName();
                        $attachment['fingerprint'] = $notAnnotatedAttachment->getFingerprint();
                    }
                }
