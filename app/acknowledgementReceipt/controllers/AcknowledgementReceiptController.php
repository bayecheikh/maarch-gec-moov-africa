...
class AcknowledgementReceiptController
{
        ...
        foreach ($acknowledgementReceiptsModel as $acknowledgementReceipt) {
            $contact = ContactModel::getById(
                [
                    ...
                    'select' => ['firstname', 'lastname', 'company', 'email', 'is_confidential']
                ]
            );
            ...
            if (!$contact['is_confidential'] || $canViewConfidentialContactInformation) {
                $contact['email']['value'] = $contactEmail;
            }
            unset($contact['is_confidential']);

            $contact['labelToDisplay'] = (!empty($contact['lastname']))
                ? $contact['firstname'] . ' ' . $contact['lastname']
                : $contact['company'];
            $contact['id'] = $acknowledgementReceipt['contact_id'];

            $userLabel = UserModel::getLabelledUserById(['id' => $acknowledgementReceipt['user_id']]);

            $currentAcknowledgementReceipt = [
            ...
            $acknowledgementReceipts[] = $this->reformatAckReturnUsingConfidentiality($currentAcknowledgementReceipt);
        }
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        $acknowledgementReceipt = $this->reformatAckReturnUsingConfidentiality($acknowledgementReceipt);
     ...
     * @throws Exception
     */
    private function reformatAckReturnUsingConfidentiality(array $ack): array
    {
        $returnAck = $ack;
        $canViewConfidentialContactInformation = PrivilegeController::hasPrivilege([
            'privilegeId' => 'view_confidential_contact_information',
        ...
        foreach ($returnAck['cc'] as $key => $recipient) {
            if ($recipient['type'] == 'user') {
                $returnAck['cc'][$key]['labelToDisplay'] =
                    UserModel::getLabelledUserById(['id' => $recipient['id']]);
                unset($returnAck['cc'][$key]['confidential']);
                ...
                $isConfidential = $contact['is_confidential'];

                $returnAck['cc'][$key]['email'] = [
                    'confidential' => $isConfidential
                ];
                if (!$isConfidential || $canViewConfidentialContactInformation) {
                    $returnAck['cc'][$key]['email']['value'] = $recipient['email'] ?? $contact['email'];
                }
                $returnAck['cc'][$key]['labelToDisplay'] = (!empty($contact['lastname']))
                    ? $contact['firstname'] . ' ' . $contact['lastname']
                    : $contact['company'];
        ...
        foreach ($returnAck['cci'] as $key => $recipient) {
            if ($recipient['type'] == 'user') {
                $returnAck['cci'][$key]['labelToDisplay'] =
                    UserModel::getLabelledUserById(['id' => $recipient['id']]);

                unset($returnAck['cci'][$key]['confidential']);
            } elseif ($recipient['type'] == 'contact') {
                $contact = ContactModel::getById([
                    'select' => ['firstname', 'lastname', 'company', 'email', 'is_confidential'],
                    'id'     => $recipient['id']
                ...
                $returnAck['cci'][$key]['email'] = [
                    'confidential' => $isConfidential
                ];
                if (!$isConfidential || $canViewConfidentialContactInformation) {
                    $returnAck['cci'][$key]['email']['value'] = $recipient['email'] ?? $contact['email'];
                }
                $returnAck['cci'][$key]['labelToDisplay'] = (!empty($contact['lastname']))
                    ? $contact['firstname'] . ' ' . $contact['lastname']
                    : $contact['company'];
