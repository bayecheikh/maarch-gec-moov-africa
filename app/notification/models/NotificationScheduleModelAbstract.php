...
abstract class NotificationScheduleModelAbstract
{
    ...
    public static function getCrontab(array $aArgs = []): array
    {
        ...
        foreach ($lines as $cronLine) {
            ...
            if ($cronLine[0] == '@') {
                ...
                $parts = explode(' ', $cronLine, 6);
                if (count($parts) >= 6) {
                    list($m, $h, $dom, $mon, $dow, $cmd) = $parts;
                } else {
                    continue;
                }
            }
    ...
    public static function createScriptNotification(array $aArgs): bool
    {
        ...
        fwrite($file_open, "\n");
        if ($event_id == 'baskets') {
            //Need to replace 'basket_event_stack.php' with 'BasketEventStackNotificationScript.php'
            $filename = str_contains($notification_id, 'MCM') ?
                'MaarchCourrierMobileNotificationScript.php' : 'basket_event_stack.php';
            fwrite($file_open, "php '$filename' -c $ConfigNotif -n $notification_id");
        } elseif (
        ...
        if (!str_contains($notification_id, 'MCM')) {
            fwrite($file_open, "\n");
            fwrite($file_open, 'cd $path');
            fwrite($file_open, "\n");
