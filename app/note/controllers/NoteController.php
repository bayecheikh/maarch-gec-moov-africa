...
class NoteController
{
    ...
    public static function getTemplates(Request $request, Response $response): Response
    {
        ...
        if (!empty($query['resId']) && is_numeric($query['resId'])) {
            ...
            $currentUserEntities = UserModel::getEntitiesById(
                ['id' => $GLOBALS['id'], 'select' => ['entities.entity_id']]
            );
            if (!empty($currentUserEntities)) {
                $currentUserEntities = array_column($currentUserEntities, 'entity_id');

                $templates = TemplateModel::getWithAssociation([
                    'select'  => ['DISTINCT(templates.template_id), template_label', 'template_content'],
                    'where'   => ['template_target = ?', 'value_field IN (?)'],
                    'data'    => ['notes', $currentUserEntities],
                    'orderBy' => ['template_label']
                ]);
            } else {
                $templates = TemplateModel::getByTarget([
