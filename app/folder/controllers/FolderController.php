...
class FolderController
{
    ...
    public function get(Request $request, Response $response): Response
    {
        ...
        foreach ($folders as $folder) {
            ...
            $folderScope = FolderController::getScopeFolders(
                ['login' => $GLOBALS['login'], 'folderId' => $folder['id'], 'edition' => true]
            );
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        if ($folder['public']) {
            $entitiesFolder = EntityFolderModel::getEntitiesByFolderId(
                [
                    'folder_id' => $args['id'],
                    'select'    => ['entities_folders.entity_id', 'entities_folders.edition', 'entities.entity_label']
                ]
            );
            $canDeleteWithChildren = FolderController::areChildrenInPerimeter(
                ['folderId' => $args['id'], 'entities' => $userEntities]
            );
            foreach ($entitiesFolder as $value) {
                ...
                $folder['sharing']['entities'][] = [
                    ...
                    'canDelete' => $canDelete,
                    'label'     => $value['entity_label']
                ];
            }

            $keywordsFolder = EntityFolderModel::getKeywordsByFolderId(
                ['folder_id' => $args['id'], 'select' => ['edition', 'keyword']]
            );
    ...
    public function create(Request $request, Response $response): Response
    {
        ...
        if (empty($data['parent_id'])) {
            ...
            $folder = FolderController::getScopeFolders(
                ['login' => $GLOBALS['login'], 'folderId' => $data['parent_id'], 'edition' => true]
            );
            if (empty($folder[0])) {
                return $response->withStatus(400)->withJson(
        ...
        if ($public && !empty($data['parent_id'])) {
            $entitiesSharing = EntityFolderModel::getEntitiesByFolderId(
                ['folder_id' => $data['parent_id'], 'select' => ['entities.id', 'entities_folders.edition']]
            );
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        if (!empty($data['parent_id']) && $data['parent_id'] == $args['id']) {
            return $response->withStatus(400)->withJson(['errors' => 'Parent_id and id can not be the same']);
        }
        if (
            !empty($data['parent_id']) &&
            FolderController::isParentFolder(['parent_id' => $data['parent_id'], 'id' => $args['id']])
        ) {
            return $response->withStatus(400)->withJson(
        ...
        $folder = FolderController::getScopeFolders(
            ['login' => $GLOBALS['login'], 'folderId' => $args['id'], 'edition' => true]
        );
        if (empty($folder[0])) {
            return $response->withStatus(400)->withJson(
                ['errors' => 'Folder not found or out of your perimeter']
        ...
        if (empty($data['parent_id'])) {
            ...
            $folderParent = FolderController::getScopeFolders(
                ['login' => $GLOBALS['login'], 'folderId' => $data['parent_id'], 'edition' => true]
            );
        ...
        if ($folder[0]['parent_id'] != $data['parent_id']) {
            ...
            $childrenInPerimeter = FolderController::areChildrenInPerimeter(
                ['folderId' => $args['id'], 'entities' => $userEntities]
            );
     ...
     * @throws Exception
     */
    public function sharing(Request $request, Response $response, array $args): Response
    {
        $data = $request->getParsedBody();
        ...
        $entitiesBefore = EntityFolderModel::getEntitiesByFolderId(
            ['select' => ['entities_folders.entity_id, edition'], 'folder_id' => $args['id']]
        );
     ...
     * @throws Exception
     */
    public function folderSharing(array $args = []): bool
    {
        $folder = FolderController::getScopeFolders(
            ['login' => $GLOBALS['login'], 'folderId' => $args['folderId'], 'edition' => true]
        );
        if (empty($folder[0])) {
            return false;
        }
        ...
        if (!empty($entitiesToAdd)) {
            $alreadyPresentEntities = EntityFolderModel::get(
                [
                    'select' => ['entity_id'],
                    'where'  => ['folder_id = ?', 'entity_id in (?)'],
                    'data'   => [$args['folderId'], $entitiesToAdd]
                ]
            );
        ...
        if (!empty($entitiesToRemove)) {
            EntityFolderModel::delete(
                ['where' => ['entity_id in (?)', 'folder_id = ?'], 'data' => [$entitiesToRemove, $args['folderId']]]
            );
        }
        if (!empty($args['add'])) {
    ...
    public function delete(Request $request, Response $response, array $aArgs): Response
    {
        ...
        $canDelete = FolderController::areChildrenInPerimeter(
            ['folderId' => $aArgs['id'], 'entities' => $userEntities]
        );
        ...
        $folder = FolderController::getScopeFolders(
            ['login' => $GLOBALS['login'], 'folderId' => $aArgs['id'], 'edition' => true]
        );
     ...
     * @throws Exception
     */
    public static function folderDeletion(array $args = []): bool
    {
        $folder = FolderController::getScopeFolders(
            ['login' => $GLOBALS['login'], 'folderId' => $args['folderId'], 'edition' => true]
        );
     ...
     * @throws Exception
     */
    public static function areChildrenInPerimeter(array $args = []): bool
    {
        ...
        // All sub-folders of a folders have the same owner user -> if user is owner of folder,
        // all children are in perimeter
        ...
        if (!empty($children)) {
            foreach ($children as $child) {
                ...
                if (
                    !FolderController::areChildrenInPerimeter(
                        ['folderId' => $child['id'], 'entities' => $args['entities']]
                    )
                ) {
                    return false;
                }
     ...
     * @throws Exception
     */
    public function getResourcesById(Request $request, Response $response, array $args): Response
    {
        if (!Validator::numericVal()->notEmpty()->validate($args['id'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Route id is not an integer']);
        }
        ...
        $foldersResources = ResourceFolderModel::get(
            ['select' => ['res_id'], 'where' => ['folder_id = ?'], 'data' => [$args['id']]]
        );
        ...
        if (!empty($foldersResources)) {
            ...
            !is_numeric($queryParams['limit']) ? 10 : (int)$queryParams['limit']);
            ...
            $resIds = ResourceListController::getIdsWithOffsetAndLimit(
                ['resources' => $rawResources, 'offset' => $queryParams['offset'], 'limit' => $queryParams['limit']]
            );
            ...
            if (!empty($resIds)) {
                ...
                $adminFolder = ConfigurationModel::getByPrivilege([
                    'privilege' => 'admin_folders',
                    'select'    => ['value']
                ]);

                if (!empty($adminFolder)) {
                    $configuration = json_decode($adminFolder['value'], true);
                    ...
                    $tableFunction = $selectData['tableFunction'];
                    $leftJoinFunction = $selectData['leftJoinFunction'];
                } else {
                    $listDisplay = ['folders'];
                ...
                $columnsToDelete = ['res_letterbox.confidentiality', 'res_letterbox.integrations'];
                $select = array_diff($select, $columnsToDelete);
                ...
                $followedResources = UserFollowedResourceModel::get(
                    ['select' => ['res_id'], 'where' => ['user_id = ?'], 'data' => [$GLOBALS['id']]]
                );
                ...
                $folderPrivilege = PrivilegeController::hasPrivilege(
                    ['privilegeId' => 'include_folders_and_followed_resources_perimeter', 'userId' => $GLOBALS['id']]
                );
                foreach ($formattedResources as $key => $formattedResource) {
                    if ($folderPrivilege) {
                        $formattedResources[$key]['allowed'] = true;
                    } else {
                        $formattedResources[$key]['allowed'] = ResController::hasRightByResId(
                            ['resId' => [$formattedResource['resId']], 'userId' => $GLOBALS['id']]
                        );

                        if (!$formattedResources[$key]['allowed']) {
                            $formattedResources[$key]['display'] = [];
                        }
                    }
                }
            }
     ...
     * @throws Exception
     */
    public function addResourcesById(Request $request, Response $response, array $args): Response
    {
        if (!Validator::numericVal()->notEmpty()->validate($args['id'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Route id is not an integer']);
        }
        ...
        $resourcesInfo = ResModel::get(
            ['select' => ['alt_identifier'], 'where' => ['res_id in (?)'], 'data' => [$resourcesToClassify]]
        );
        ...
        HistoryController::add([
            ...
            'recordId'  => $args['id'],
            'eventType' => 'ADD',
            'info'      => _FOLDER_RESOURCES_ADDED . " : " . implode(", ", $resourcesIdentifier) . " " .
                _FOLDER_TO_FOLDER . " \"" . $folders['label'] . "\"",
            'moduleId'  => 'folder',
     ...
     * @throws Exception
     */
    public function removeResourcesById(Request $request, Response $response, array $args): Response
    {
        if (!Validator::numericVal()->notEmpty()->validate($args['id'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Route id is not an integer']);
        }
        ...
        $resourcesInfo = ResModel::get(
            ['select' => ['alt_identifier'], 'where' => ['res_id in (?)'], 'data' => [$resourcesToUnclassify]]
        );
        $resourcesIdentifier = array_column($resourcesInfo, 'alt_identifier');
        ...
        HistoryController::add([
            ...
            'recordId'  => $args['id'],
            'eventType' => 'DEL',
            'info'      => _FOLDER_RESOURCES_REMOVED . " : " . implode(", ", $resourcesIdentifier) . " " .
                _FOLDER_TO_FOLDER . " \"" . $folder['label'] . "\"",
            'moduleId'  => 'folder',
     ...
     * @throws Exception
     */
    public static function getScopeFolders(array $args): array
    {
        $login = $args['login'];

        $user = UserModel::getByLogin(['login' => $login, 'select' => ['id']]);
        $userEntities = EntityModel::getWithUserEntities(
            ['select' => ['entities.id'], 'where' => ['user_id = ?'], 'data' => [$user['id']]]
        );
     ...
     * @return Response
     */
    public function pinFolder(Request $request, Response $response, array $args): Response
    {
        if (!Validator::numericVal()->notEmpty()->validate($args['id'])) {
     ...
     * @throws Exception
     */
    public function getPinnedFolders(Request $request, Response $response): Response
    {
     ...
     * @return Response
     */
    public function unpinFolder(Request $request, Response $response, array $args): Response
    {
        if (!Validator::numericVal()->notEmpty()->validate($args['id'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Route id not found or is not an integer']);
        }
     ...
     * @throws Exception
     */
    public static function hasFolders(array $args): bool
    {
     ...
     * @throws Exception
     */
    private static function isParentFolder(array $args): bool
    {
    ...
    /**
     * @param int $parentId
     * @param int $levelParent
     * @param int $folderOwner
     * @return void
     * @throws Exception
     */
    private static function updateChildren(int $parentId, int $levelParent, int $folderOwner): void
    {
    ...
    {
        $configuration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_folders']);
