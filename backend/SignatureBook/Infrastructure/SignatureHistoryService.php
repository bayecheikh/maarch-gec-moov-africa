...
class SignatureHistoryService implements SignatureHistoryServiceInterface
{
    public function historySignatureValidation(int $resId, ?int $resIdMaster): void
    {
        if (!empty($resIdMaster)) {
            $attachment = AttachmentModel::getById(['id' => $resId, 'select' => ['title', 'identifier']]);
            ...
            HistoryController::add([
                ...
                'info'      => _ATTACHMENT_SIGNED . " : " . $attachment['title'] . " - " . $attachment['identifier']
            ]);
            ...
            $mainResource = ResModel::getById(['resId' => $resId, 'select' => ['subject', 'alt_identifier']]);
            HistoryController::add([
                'tableName' => 'res_letterbox',
                'recordId'  => $resId,
                'eventType' => 'SIGN',
    ...
    {
        if (!empty($resIdMaster)) {
            $attachment = AttachmentModel::getById(['resId' => $resId, 'select' => ['title', 'identifier']]);
            HistoryController::add([
                'tableName' => 'res_letterbox',
                'recordId'  => $resIdMaster,
                'eventType' => 'SIGN',
