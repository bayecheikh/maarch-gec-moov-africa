...
class MaarchParapheurSignatureService implements SignatureServiceInterface
{
    ...
    {
        $response = CurlModel::exec([
            'url'        => rtrim($this->config->getUrl(), '/') . '/rest/documents/' . $documentId . '/actions/2',
            'bearerAuth' => ['token' => $accessToken],
            'headers'    => [
                'content-type: application/json',
                'Accept: application/json'
        ...
        if ($response['code'] >= 400 || $response['code'] < 200) {
            $errorMessage = $response['response']['errors'] ??
                $response['errors'] ??
                'Error occurred while refusing the signature';
    ...
    {
        $response = CurlModel::exec([
            'url'        => rtrim($this->config->getUrl(), '/') . '/rest/documents/' . $documentId . '/actions/3',
            'bearerAuth' => ['token' => $accessToken],
            'headers'    => [
                'content-type: application/json',
                'Accept: application/json'
        ...
        if ($response['code'] >= 400 || $response['code'] < 200) {
            $errorMessage = $response['response']['errors'] ??
                $response['errors'] ??
                'Error occurred while reverting visa workflow';
    ...
    {
        $response = CurlModel::exec([
            'url'       => rtrim($this->config->getUrl(), '/') . '/rest/version',
            'basicAuth' => [
        ...
        return $response['response'] ?? ['errors' => 'No response from Parapheur'];
    }
}
