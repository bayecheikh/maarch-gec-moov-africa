...
class CreateAndUpdateUserInSignatoryBook
{
    ...
    public function createAndUpdateUser(UserInterface $user): UserInterface
    {
        ...
        if (!empty($externalId)) {
            $externalIdParapheur = $user->getExternalId()['internalParapheur'] ?? null;
            if (!empty($externalIdParapheur) && $this->signatureBookUserService->doesUserExists($externalIdParapheur)) {
