...
class AutoCompleteController
{
    ...
    public static function getUsers(Request $request, Response $response): Response
    {
        ...
        $users = UserModel::get([
            'select'  => ['id', 'user_id', 'firstname', 'lastname', 'status'],
     ...
     * @throws Exception
     */
    public static function getMaarchParapheurUsers(Request $request, Response $response): Response
    {
        ...
        $excludedUsers = [];
        if (!empty($data['excludeAlreadyConnected'])) {
        ...
        if ($loadedXml->signatoryBookEnabled == 'maarchParapheur') {
            ...
            foreach ($curlResponse['response'] ?? [] as $key => $value) {
                if (!empty($data['excludeAlreadyConnected']) && in_array($value['id'], $excludedUsers)) {
     ...
     * @throws Exception
     */
    public function getFastParapheurUsers(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public static function getCorrespondents(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        ...
        if (empty($queryParams['noContacts'])) {
            ...
            foreach ($searchableParameters as $searchableParameter) {
                if (str_contains($searchableParameter['identifier'], 'contactCustomField_')) {
            ...
            $contacts = ContactModel::get([
                'select'  => ['id', 'email', 'is_confidential'],
                'where'   => $requestData['where'],
                'data'    => $requestData['data'],
            ...
            foreach ($contacts as $contact) {
                ...
                $hasService = PrivilegeController::hasPrivilege([
                    'privilegeId' => 'admin_contacts',
                    'userId'      => $GLOBALS['id']
                ]);

                if (
                    $contact['is_confidential'] &&
                    !$canViewConfidentialContactInformation &&
                    !$hasService &&
                    $searchOnEmails
                ) {
                    continue;
                ...
                if ($searchOnEmails && empty($autoContact['email'])) {
                    $autoContact['email'] = [
                        'value'        => !$contact['is_confidential'] ||
                        ($canViewConfidentialContactInformation || $hasService) ? $emailValue : null,
                        'confidential' => $contact['is_confidential']
                    ];
                }
        ...
        if ($total > $limit) {
            ...
            $autocompleteContacts = array_slice(
                $autocompleteContacts,
                0,
                (int)round(count($autocompleteContacts) / (int)$divider)
            );
            $autocompleteUsers = array_slice(
                $autocompleteUsers,
                0,
                (int)round(count($autocompleteUsers) / (int)$divider)
            );
            $autocompleteEntities = array_slice(
                $autocompleteEntities,
                0,
                (int)round(count($autocompleteEntities) / (int)$divider)
            );
            $autocompleteContactsGroups = array_slice(
                $autocompleteContactsGroups,
                0,
                (int)round(count($autocompleteContactsGroups) / (int)$divider)
            );
        }
     ...
     * @throws Exception
     */
    public static function getUsersForAdministration(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
        $check = Validator::stringType()->notEmpty()->validate($data['search']);
        if (!$check) {
     ...
     * @throws Exception
     */
    public static function getUsersForCircuit(Request $request, Response $response): Response
    {
        $env = new Environment();
        $queryParams = $request->getQueryParams();
            ...
            foreach ($users as $value) {
                ...
                if (isset($externalId['internalParapheur'])) {
                    ...
                    $entity = UserModel::getPrimaryEntityById(
                        ['id' => $value['id'], 'select' => ['entities.short_label']]
                    );
                    $data[] = [
                        'type'        => 'user',
                        'id'          => $value['id'],
     ...
     * @throws Exception
     */
    public static function getEntities(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        if (!Validator::stringType()->notEmpty()->validate($queryParams['search'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Bad Request']);
     ...
     * @throws Exception
     */
    public static function getStatuses(Request $request, Response $response): Response
    {
     ...
     * @throws Exception
     */
    public static function getContacts(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public static function getContactsCompany(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        ...
        if (!empty($contacts)) {
            ...
            $isConfidentialContact = $contacts[0]['is_confidential'];

            if (!$canViewConfidentialContactInformation && $isConfidentialContact) {
                unset($contacts[0]['addressNumber']);
                unset($contacts[0]['addressStreet']);
                unset($contacts[0]['addressAdditional1']);
                unset($contacts[0]['addressAdditional2']);
                unset($contacts[0]['addressPostcode']);
                unset($contacts[0]['addressTown']);
                unset($contacts[0]['addressCountry']);
            }
            unset($contacts[0]['is_confidential']);
        }
     ...
     * @throws Exception
     */
    public static function getContactsByName(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        ...
        //Delete informations for confidential contact
        $canViewConfidentialContactInformation = PrivilegeController::hasPrivilege([
            'privilegeId' => 'view_confidential_contact_information',
            'userId'      => $GLOBALS['id']
        ]);
        foreach ($contacts as $key => $contact) {
            $isConfidentialContact = $contact['is_confidential'];

            if (!$canViewConfidentialContactInformation && $isConfidentialContact) {
                unset($contacts[$key]['addressNumber']);
                unset($contacts[$key]['addressStreet']);
                unset($contacts[$key]['addressAdditional1']);
                unset($contacts[$key]['addressAdditional2']);
                unset($contacts[$key]['addressPostcode']);
                unset($contacts[$key]['addressTown']);
                unset($contacts[$key]['addressCountry']);
            }
            unset($contacts[$key]['is_confidential']);
        }
     ...
     * @throws Exception
     */
    public static function getBanAddresses(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public static function getOuM2MAnnuary(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public static function getAvailableContactsForM2M(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        if (!Validator::stringType()->notEmpty()->validate($queryParams['search'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Query params search is empty']);
     ...
     * @throws Exception
     */
    public static function getBusinessIdM2MAnnuary(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public static function getFolders(Request $request, Response $response): Response
    {
        $data = $request->getQueryParams();
     ...
     * @throws Exception
     */
    public function getPostcodes(Request $request, Response $response): Response
    {
        $queryParams = $request->getQueryParams();
        ...
        $postcodes = array_values(array_filter($postcodes, function ($code) use ($searchPostcode, $searchTowns) {
            ...
            foreach ($searchTowns as $searchTown) {
                if ($searchTown == 'ST' || $searchTown == 'SAINT') {
                    if (
                        !str_contains($code['label'], 'ST')
                        && !str_contains($code['label'], 'SAINT')
                    ) {
                        $townFound = false;
                        break;
                    }
                    ...
                    if (
                        !str_contains($code['label'], 'STE')
                        && !str_contains($code['label'], 'SAINTE')
                    ) {
                        $townFound = false;
                        break;
                    }
                } elseif (!str_contains($code['label'], $searchTown)) {
                    $townFound = false;
                    break;
                }
            }
            return $townFound || (!empty($searchPostcode) && str_starts_with($code['postcode'], $searchPostcode));
        }));
     ...
     * @throws Exception
     */
    public static function getDataForRequest(array $args): array
    {
     ...
     * @throws Exception
     */
    public static function getInsensitiveFieldsForRequest(array $args): string
    {
