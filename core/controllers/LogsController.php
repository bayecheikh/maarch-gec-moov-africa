...
class LogsController implements LogsInterface
{
     ...
     * @param bool $isCustomFormatLineUse
    ...
    public static function initMonologLogger(
        ...
        if ($isCustomFormatLineUse) {
            $formatter = new LineFormatter($output, $dateFormat);
            $streamHandler->setFormatter($formatter);
        }

        $loggerName = $name ?: 'SCRIPT';
    ...
    {
        $configPaths = ['config/config.json', 'config/config.json.default'];
    ...
    {
        $maxFileSize = null;
        $numberPart = substr($value, 0, strlen($value) - 2);
        $suffix = strtoupper(substr($value, -2));

        switch ($suffix) {
            case 'KB':
                $maxFileSize = (int)$numberPart * 1024;
                break;
            case 'MB':
                $maxFileSize = (int)$numberPart * 1024 * 1024;
                break;
            case 'GB':
                $maxFileSize = (int)$numberPart * 1024 * 1024 * 1024;
                break;
            default:
                if (is_numeric($value)) {
