...
use MaarchCourrier\Authentication\Infrastructure\ProConnect\ProConnectAuthenticationFactory;
...
class AuthenticationController
{
    ...
    public function getInformations(Request $request, Response $response): Response
    {
        ...
        if (!empty($loadedXml)) {
            if (!empty((string)$loadedXml->signatoryBookEnabled)) {
                ...
                if ($externalSignatoryBook['id'] == 'maarchParapheur' || $externalSignatoryBook['id'] == 'goodflag') {
                    $externalSignatoryBook['integratedWorkflow'] = true;

                    if ($externalSignatoryBook['id'] == 'goodflag') {
                        ...
                        $externalSignatoryBook['optionOtp'] =
                            $goodflagRetrieveConfiguration->getOptions()['optionOtp'] ?? false;
                    }
                } else {
        ...
        $proconnectInformations = ConfigurationModel::getByPrivilege(
            [
                'privilege' => 'admin_proconnect',
                'select'    => ['value']
            ]
        );
        if (!empty($proconnectInformations['value'])) {
            $proconnectConfiguration = json_decode($proconnectInformations['value'], true);
            if ($proconnectConfiguration['enabled']) {
                $urlProconnect = $proconnectConfiguration['url'];
                $parsedUrl = parse_url($urlProconnect);
                $return['proconnect'] = [
                    'enabled'      => $proconnectConfiguration['enabled'],
                    'url'          => isset($parsedUrl['scheme']) ?
                        $proconnectConfiguration['url'] :
                        'https://' . $proconnectConfiguration['url'],
                    'scope'        => 'openid given_name usual_name email',
                    'clientId'     => PasswordController::decrypt(
                        [
                        'encryptedData' => $proconnectConfiguration['clientId']
                        ]
                    ),
    ...
    public function authenticate(Request $request, Response $response): Response
    {
        ...
        $proConnectAuthentication = false;
        if (!empty($body['mode']) && $body['mode'] == 'proconnect') {
            $proConnectAuthentication = true;
        }

        $loggingMethod = CoreConfigModel::getLoggingMethod();
        if (in_array($loggingMethod['id'], ['standard', 'ldap']) && !$proConnectAuthentication) {
            if (
        ...
        if ($loggingMethod['id'] == 'standard' && !$proConnectAuthentication) {
            ...
            $ssoIdentity = $authenticated['sso_identity'] ?? $authenticated['login'];
            if (!AuthenticationController::isUserAuthorized(['login' => $login])) {
                return $response->withStatus(403)->withJson(['errors' => 'Authentication Failed']);
            }
        } elseif ($loggingMethod['id'] == 'keycloak') {
            ...
            session_start();
            $_SESSION['keycloak_identity'] = $ssoIdentity;
        } elseif ($loggingMethod['id'] == 'sso') {
            ...
            $_SESSION['azure_saml_identity'] = $ssoIdentity;
        } elseif ($proConnectAuthentication) {
            $authorizationCode = $body['code'];
            $nonce = $body['nonce'];

            $proConnectAuthenticationAction = ProConnectAuthenticationFactory::create();

            $user = $proConnectAuthenticationAction->execute($authorizationCode, $nonce);
            if (!empty($user)) {
                $login = $user->getLogin();
                if (!AuthenticationController::isUserAuthorized(['login' => $login])) {
                    return $response->withStatus(403)->withJson(['errors' => 'Authentication Failed']);
                }
                $proConnectIdToken = $proConnectAuthenticationAction->getIdToken();
            }
        } else {
        ...
        if ($ssoIdentity !== null) {
            $GLOBALS['sso_identity'] = $ssoIdentity;
        }
        ...
        if ($proConnectAuthentication) {
            $response = $response->withHeader('Proconnect-idToken', $proConnectIdToken);
        }
    ...
    public function logout(Request $request, Response $response): Response
    {
        ...
        if ($loggingMethod['id'] == 'cas') {
            ...
            $disconnection = AuthenticationController::ssoDisconnection();
            $logoutUrl = $disconnection['logoutUrl'];
        }
    ...
    private static function ssoConnection(): array
    {
        ...
        if (!empty($_SERVER['HTTP_' . strtoupper($mapping['login'])])) {
            ...
            $login = $_SERVER[$mapping['login']] ?? null;
        }
        ...
        // Récupération de l'identité SSO pour la correspondance JWT
        $ssoIdentity = $login; // Par défaut, utilise le login

        // Si un mapping spécifique pour l'identité SSO existe dans la configuration
        if (!empty($mapping['sso_identity'])) {
            if (!empty($_SERVER['HTTP_' . strtoupper($mapping['sso_identity'])])) {
                $ssoIdentity = $_SERVER['HTTP_' . strtoupper($mapping['sso_identity'])];
            } else {
                $ssoIdentity = $_SERVER[$mapping['sso_identity']] ?? null;
            }

            if (empty($ssoIdentity)) {
                $ssoIdentity = $headers[strtoupper($mapping['sso_identity'])] ?? $login;
            }
        }
     ...
     * @throws Exception
     */
    private static function ssoDisconnection(): array
    {
        $ssoConfiguration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_sso', 'select' => ['value']]);
        if (empty($ssoConfiguration['value'])) {
            return ['errors' => 'Sso configuration missing'];
        ...
        $ssoConfiguration = json_decode($ssoConfiguration['value'], true);
        $logoutUrl = $ssoConfiguration['ssoLogoutUrl'];

        $logoutUrl = empty($logoutUrl) ? null : $ssoConfiguration['ssoLogoutUrl'];

        if (!empty($logoutUrl) && session_status() === PHP_SESSION_ACTIVE) {
            session_destroy();
        }
        ...
        if (!empty($_COOKIE['mod_auth_openidc_session'])) {
...
*/
        ...
        try {
            $token = new AccessToken($token->jsonSerialize());
            ...
            $externalId = json_decode($userMaarch['external_id'], true) ?: [];
            unset($externalId['keycloakAccessToken']);

            if (!empty($idToken)) {
                $externalId['keycloakIdToken'] = $idToken;
            }
            if (!empty($refreshToken)) {
                $externalId['keycloakRefreshToken'] = $refreshToken;
            }

            UserModel::updateExternalId(['id' => $userMaarch['id'], 'externalId' => json_encode($externalId)]);
            ...
            if ($idToken) {
                ...
                if (count($idTokenParts) === 3) {
                    ...
                    $ssoIdentity = $payload['sub'] ?? null;
                }
            }

            return ['login' => $login, 'sso_identity' => $ssoIdentity ?? $login];
        } catch (Exception) {
            return ['errors' => 'Authentication Failed'];
        }
    }
    ...
    private static function keycloakDisconnection(): array
    {
        ...
        $idToken = $externalId['keycloakIdToken'] ?? null;
        $refreshToken = $externalId['keycloakRefreshToken'] ?? null;

        $logoutOptions = [];
        // The redirectUri from config should be registered as a valid post-logout redirect URI in Keycloak.
        if (!empty($keycloakConfig['redirectUri'])) {
            $logoutOptions['post_logout_redirect_uri'] = $keycloakConfig['redirectUri'];
        }
        if (!empty($idToken)) {
            $logoutOptions['id_token_hint'] = $idToken;
        }

        $url = $provider->getLogoutUrl($logoutOptions);

        if ($refreshToken) {
            try {
                $revocationEndpoint = rtrim(
                ...
                // Optional: log the error but don't block the logout flow
                error_log('Keycloak refresh token revocation failed: ' . $e->getMessage());
            }
        }

        unset($externalId['keycloakIdToken'], $externalId['keycloakRefreshToken']);
    ...
    private static function azureSamlConnection(): array
    {
        ...
        include_once $libDir . 'simplesamlphp/lib/_autoload.php';
    ...
    public static function getJWT(): string
    {
        ...
        if (!empty($GLOBALS['sso_identity'])) {
            $token['sso_identity'] = $GLOBALS['sso_identity'];
        }
