...
class CurlModel
{
    ...
    public static function execSOAP(array $aArgs): array
    {
        ...
        if (!empty($aArgs['options'][CURLOPT_HEADER])) {
            ...
            if (empty($body)) {
                ...
                // we remove the 4 starting items of the array (header)
                for ($i = 0; $i < 5; $i++) {
                    array_shift($body);
                }
    ...
    public static function exec(array $args): array
    {
        ...
        $isJwt = str_contains($headers, 'Content-Type: application/jwt');
        ...
        if ($args['isXml']) {
            ...
            // If we cannot parse the response as an XML, we try to parse it as a JSON
        ...
        } elseif (in_array('Accept: application/pdf', $args['headers'])) {
            $formatedResponse = trim($response);
        } elseif ($isJwt || in_array('Accept: application/jwt', $args['headers'])) {
            $formatedResponse = trim($response);
        ...
        foreach ($args['body'] as $key => $value) {
            if (!empty($value['subvalues']) && is_array($value['subvalues'])) {
                foreach ($value['subvalues'] as $subvalue) {
                    $postData .= "--{$delimiter}\r\n";
                    if (
                        is_array($subvalue) && !empty($subvalue['isFile']) && !empty($subvalue['filename']) &&
                        !empty($subvalue['content'])
                    ) {
                        $postData .= "Content-Disposition: form-data;" .
                            " name=\"{$key}\"; filename=\"{$subvalue['filename']}\"\r\n";
                        $postData .= "\r\n{$subvalue['content']}\r\n";
                    } else {
