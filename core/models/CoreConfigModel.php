...
{
    protected static ?string $customId = null;
    ...
    public static function getCustomId(): mixed
    {
        ...
        foreach ($jsonFile as $value) {
            $configPath = $value['path'] ?? null;
            $configUri = $value['uri'] ?? null;

            // Strictness rule application:
            // We ignore the configuration if BOTH Path and Uri are set (ambiguity),
            // or if NEITHER of them is set.
            if ((!empty($configPath) && !empty($configUri)) || (empty($configPath) && empty($configUri))) {
                continue;
            }

            if (!empty($configPath) && $configPath == $path) {
                self::$customId = $value['id'];
                return self::$customId;
            } elseif (
                !empty($configUri) &&
                ($configUri == $_SERVER['HTTP_HOST'] || $configUri == $_SERVER['SERVER_ADDR'])
            ) {
                self::$customId = $value['id'];
                return self::$customId;
            }
    ...
    public static function getLanguage(): mixed
    {
        ...
        $file = CoreConfigModel::getJsonLoaded(['path' => $configPath]);
    ...
    public static function useVhostEncryptKey(): bool
    {
        ...
        return empty($config['privateKeyPath'] ?? null);
    }
    ...
    public static function getEncryptKey(): string
    {
        ...
        } elseif (is_file($encryptKeyPath) && is_readable($encryptKeyPath)) {
    ...
    public static function getMailevaConfiguration(): array
    {
        ...
        if (!empty($configuration)) {
            ...
            if (
                !empty($mailevaConfig['uri'])
                && $mailevaConfig['uri'] == MailevaConfiguration::MAILEVA_PRODUCTION_URI
            ) {
                $mailevaConfig['clientId'] = MailevaConfiguration::MAILEVA_PRODUCTION_CLIENT_ID;
                $mailevaConfig['clientSecret'] = MailevaConfiguration::MAILEVA_PRODUCTION_CLIENT_SECRET;
            }
            if (!empty($mailevaConfig['uri']) && $mailevaConfig['uri'] == MailevaConfiguration::MAILEVA_SANDBOX_URI) {
                $mailevaConfig['clientId'] = MailevaConfiguration::MAILEVA_SANDBOX_CLIENT_ID;
                $mailevaConfig['clientSecret'] = MailevaConfiguration::MAILEVA_SANDBOX_CLIENT_SECRET;
            }
        }
     ...
     * @throws Exception
     */
    public static function getXmlLoaded(array $args): SimpleXMLElement|false|null
    {
    ...
    public static function uniqueId(): string
    {
        ...
        $uniqueId .= str_pad(base_convert((string)$msec, 10, 16), 4, '0', STR_PAD_LEFT);
        $uniqueId .= str_pad(base_convert((string)mt_rand(), 10, 36), 6, '0', STR_PAD_LEFT);
    ...
    public static function getApplicationUrl(): ?string
    {
        ...
        if (!empty($customConfig['config']['maarchUrl'] ?? null)) {
            return trim($customConfig['config']['maarchUrl'], '/');
        }
        return null;
    }
}
